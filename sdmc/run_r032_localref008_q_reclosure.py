#!/usr/bin/env python3
"""
Fine Q closure around the exact-promoted lowridge021 solution.

All geometry, matter, SDMC structure, n_s and tau are fixed.
Only Q = ln(10^10 A_s) - 2 tau is varied in a narrow bracket around
the current Q-best. The purpose is to establish whether the promoted
Q-best is locally enclosed or clipped along the primordial-amplitude axis.
"""
...
OUT=Path("output/localref008_q_reclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
OFFSETS=np.array([-0.0025,-0.0020,-0.0015,-0.0010,-0.0005,
                   0.0,0.0005,0.0010,0.0015,0.0020,0.0025])
...
def run(i,dq):
    ...
    print("LOCALREF_Q_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
ok["delta_vs_lowridge_q"]=ok.chi2_planck-center
df=pd.DataFrame(rows).merge(ok[["id","delta_vs_lowridge_q"]],on="id",how="left")
df.to_csv(OUT/"localref008_q_reclosure.csv",index=False)
best=ok.nsmallest(8,"chi2_planck").to_dict("records")
summary={"Q0":Q0,"center_chi2":center,"best":best[0],
         "best_delta_vs_lowridge_q":float(ok.chi2_planck.min()-center),
...
(OUT/"localref008_q_reclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("LOCALREF_Q_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LOCALREF_Q_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
