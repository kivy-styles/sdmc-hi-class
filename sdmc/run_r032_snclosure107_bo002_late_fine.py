...
This fine scan is centered on the exact bo002 point and searches for a nearby late background that reduces the remaining SN closure deficit.\n\nThe target is not "best SN alone". It is:
...
OUT=Path("output/snclosure107_bo002_late_fine"); OUT.mkdir(parents=True,exist_ok=True)
...
EDGE024_PD_FAIR=-0.675202390114676
EDGE024_BAO_CHI=13.433005
...
print("SN107LATE_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

sob=qmc.Sobol(d=3,scramble=True,seed=107702)
u=sob.random_base2(m=7)  # 128
# Broad but physically conservative low-z box.
Alo,Ahi=0.0000,0.0200
Blo,Bhi=0.0100,0.0200
Hlo,Hhi=69.70,70.40
rows=[]
for i,x in enumerate(u):
    ...
    rows.append(r)
    if i%16==0: print("SN107LATE_PROGRESS",i,json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"snclosure107_bo002_late_stage1_all.csv",index=False)
...
short.to_csv(OUT/"snclosure107_bo002_late_stage1_shortlist.csv",index=False)
...
def planck_for(row):
    ...
    print("SN107LATE_PLANCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
pdf.to_csv(OUT/"snclosure107_bo002_late_planck_shortlist.csv",index=False)
...
(OUT/"snclosure107_bo002_late_summary.json").write_text(json.dumps(summary,indent=2))
print("SN107LATE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SN107LATE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
