...
OUT=Path("output/ztbest_qfinebest_fullplik")
...
CANDIDATES=[
    dict(id="ztbest_qfinebest",H0=70.79608815124146,ob=0.022083219194622913,
         oc=0.12299536722293603,ns=0.9625227132590487,tau=0.055202901571989066,
         lnAs=3.054869604391977,AF=0.04984375,
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("ZTQFINE_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("ZTQFINE_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("ZTQFINE_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("ZTQFINE_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("ZTQFINE_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"ztbest_qfinebest_fullplik.csv",index=False)
...
(OUT/"ztbest_qfinebest_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTQFINE_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
