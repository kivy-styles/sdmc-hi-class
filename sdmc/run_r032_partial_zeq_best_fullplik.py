...
OUT=Path("output/partial_zeq_stability_edge_fullplik")
...
CANDIDATES=[
    dict(id="partial_zeq_stability_edge",H0=70.86830361078158,ob=0.022083219194622913,
         oc=0.12299536722293603,ns=0.9625227132590487,tau=0.055202901571989066,
         lnAs=3.0598696043919773,AF=0.0488125,
         zc=2.90625,width=0.474375,dfloor=0.0443125,
         lam=18.015625,zt=17.0109375),
]
def table(path):
    import re
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("PZEQSTABEDGE_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("PZEQSTABEDGE_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("PZEQSTABEDGE_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("PZEQSTABEDGE_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("PZEQSTABEDGE_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"partial_zeq_stability_edge_fullplik.csv",index=False)
...
(OUT/"partial_zeq_stability_edge_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQSTABEDGE_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
