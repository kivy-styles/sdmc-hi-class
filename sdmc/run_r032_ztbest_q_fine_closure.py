#!/usr/bin/env python3
"""
Fine Q closure around the zt-best Q-best solution.
...
OUT=Path("output/ztbest_q_fine_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("QFINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
ok["delta_vs_qbest"]=ok.chi2_planck-center
df=pd.DataFrame(rows).merge(ok[["id","delta_vs_qbest"]],on="id",how="left")
df.to_csv(OUT/"ztbest_q_fine_closure.csv",index=False)
best=ok.nsmallest(8,"chi2_planck").to_dict("records")
summary={"Q0":Q0,"center_chi2":center,"best":best[0],
         "best_delta_vs_qbest":float(ok.chi2_planck.min()-center),
...
(OUT/"ztbest_q_fine_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("QFINE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("QFINE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
