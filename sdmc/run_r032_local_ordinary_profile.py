...
OUT=Path("output/local_ordinary")
...
print("LOCALORD_LCDM_REFERENCE",lcdm,flush=True)

# Local central-composite design in (H0, omega_b, omega_cdm) around
# the physical + primordial basin.  Primordial coordinates are held fixed
# for this diagnostic; flat closure is recomputed by sdmc_ini at every point.
steps=np.array([0.15,0.00010,0.00080],float)
design=[("center",CURRENT.copy())]
for j,name in enumerate(["H0","ob","oc"]):
    for mult in (-2,-1,1,2):
        x=CURRENT.copy()
        x[j]+=mult*steps[j]
        design.append((f"{name}_{mult:+d}",x))
for a in (-1,1):
    for b in (-1,1):
        for c in (-1,1):
            x=CURRENT.copy()
            ...
            x[1]+=b*steps[1]
            x[2]+=c*steps[2]
            design.append((f"corner_{a:+d}_{b:+d}_{c:+d}",x))
...
(OUT/"local_ordinary_design.json").write_text(json.dumps(meta,indent=2))
...
for ic,(tag,x) in enumerate(design,1):
    ...
    print("LOCALORD_POINT",json.dumps(rec,sort_keys=True),flush=True)
    rows.append(rec)

df=pd.DataFrame(rows)
df.to_csv(OUT/"local_ordinary_screen.csv",index=False)
...
short.to_csv(OUT/"local_ordinary_shortlist.csv",index=False)

print("LOCALORD_BEST_PLANCK",ok.nsmallest(10,"delta_planck")[NAMES+["id","delta_planck","delta_pantheonplus","delta_union3","delta_desy5","sigma8","min_cs2_all","max_cs2_all"]].to_dict("records"),flush=True)
print("LOCALORD_BEST_PP",ok.nsmallest(10,"screen_planck_pp")[NAMES+["id","screen_planck_pp","delta_planck","delta_pantheonplus"]].to_dict("records"),flush=True)
print("LOCALORD_BEST_UNION3",ok.nsmallest(10,"screen_planck_union3")[NAMES+["id","screen_planck_union3","delta_planck","delta_union3"]].to_dict("records"),flush=True)
print("LOCALORD_BEST_DESY5",ok.nsmallest(10,"screen_planck_desy5")[NAMES+["id","screen_planck_desy5","delta_planck","delta_desy5"]].to_dict("records"),flush=True)
print("LOCALORD_SHORTLIST",short[NAMES+["id","delta_planck","delta_pantheonplus","delta_union3","delta_desy5","sigma8","rd","ell_A","min_cs2_all","max_cs2_all"]].to_dict("records"),flush=True)
