#!/usr/bin/env python3
"""
Acoustically locked ordinary-parameter re-optimization around lrlowaf001.

The structural sector is fixed at lrlowaf001.  Re-open:
...
OUT=Path("output/r032_lrlowaf001_ordinary4d_reopt"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5
TARGET=301.6798271349195

H00=71.21322578595301
OB0=0.022215084896497428
OC0=0.12228109714277088
NS0=0.9647922897702084
TAU=0.055202901571989066
Q0=2.9439509667241945

AF=0.03385626635421067
ZC=3.6682934215292335
WIDTH=0.34275636240839963
D0=0.34231919445927034
DF=0.04743026036769152
LAM=18.40625
ZT=16.24762099627405

# omega_b, omega_cdm, n_s, Q
LOW=np.array([0.02198,0.1212,0.9615,2.9405],float)
HIGH=np.array([0.02236,0.1234,0.9680,2.9465],float)
...
sam=qmc.Sobol(d=4,scramble=True,seed=20260928)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_lrlowaf001_ordinary4d_reopt.csv",index=False)
...
(OUT/"r032_lrlowaf001_ordinary4d_reopt_summary.json").write_text(json.dumps(summary,indent=2))
