#!/usr/bin/env python3
"""
Bayesian structural recovery around the SN-aware sobol233 late background.

Low-z geometry is frozen at the SN-friendly point:
  A_late=0.0382072245329618
  B_late=0.0173954053828492
  H0=70.18422456160188
...
OUT=Path("output/snclosure233_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
AL=0.0382072245329618; BL=0.0173954053828492
...
NEW_BAO=14.044965507328229
SN_PP=0.6269898442551494
SN_U3=0.6747059034969425
SN_D5=1.3310749605298042
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SN233BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure233_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"exact promotion should achieve P+D fair < -0.674706 for Pantheon+ and Union3 closure; BO optimizes Planck-lite while raw DESI is checked on promoted points"}
(OUT/"snclosure233_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SN233BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
