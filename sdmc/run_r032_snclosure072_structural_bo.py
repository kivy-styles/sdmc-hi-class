...
Low-z geometry is frozen at the SN-friendly point:
  A_late=0.0134815868735313
  B_late=0.0181835683900862
...
OUT=Path("output/snclosure072_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
NEW_BAO=14.799708148515345
SN_PP=2.501430992037058
SN_U3=2.144542176509276
SN_D5=4.895314551889896
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SN072BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure072_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"exact promotion should achieve P+D fair < -2.50143 for Pantheon+ and Union3 closure; BO optimizes Planck-lite while raw DESI is checked on promoted points"}
(OUT/"snclosure072_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SN072BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
