...
def run_one(label,path,seeds):
    ...
    for iseed,seed in enumerate(seeds):
        ...
        print("PHYSRIDGE_FULLPLIK_SEED",label,iseed,*[row[0],row[2],row[3]],flush=True)
    ...
    print("PHYSRIDGE_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x

models=[
  ("PHYSICAL_RIDGE",OUT/"physical_ridge_00_cl_lensed.dat"),
  ("ACOUSTIC_ONLY",OUT/"acoustic_control_00_cl_lensed.dat"),
  ("LCDM",OUT/"fullplik_lcdm_00_cl_lensed.dat"),
]
...
for a,b in [("PHYSICAL_RIDGE","LCDM"),("ACOUSTIC_ONLY","LCDM"),("PHYSICAL_RIDGE","ACOUSTIC_ONLY")]:
    d=r[a]["chi2_profile_total"]-r[b]["chi2_profile_total"]
    print("PHYSRIDGE_FULLPLIK_DELTA",a,"MINUS",b,d,flush=True)

fields=list(results[0].keys())
with (OUT/"physical_ridge_fullplik_profile.csv").open("w",newline="") as f:
