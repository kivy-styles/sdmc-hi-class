#!/usr/bin/env python3
"""
Focused micro-late SN-aware closure screen around the exact sobol076 basin with BO002 structure frozen.
...
OUT=Path("output/snclosure107_bo002_sobol076_micro"); OUT.mkdir(parents=True,exist_ok=True)
...
A0=0.01686786537989974
B0=0.014543195590376853
H00=70.00876109628007
EDGE024_PD_FAIR=-0.6598910151995483
EDGE024_BAO_CHI=13.40
...
EDGE024_BAO_CHI=center["bao_chi2"]
print("SNCLOSE_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

sob=qmc.Sobol(d=3,scramble=True,seed=760107)
u=sob.random_base2(m=8)  # 256
# Micro-search through the low-H side of the sobol076 basin, where only ~0.2 joint chi2 remains to close.
Alo,Ahi=0.0135,0.0195
Blo,Bhi=0.0130,0.0160
Hlo,Hhi=69.86,70.03
rows=[]
