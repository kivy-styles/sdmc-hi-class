...
OUT=Path("output/snclosure107_bo002_sobol036_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
Q=2.94347100773082; AS=math.exp(Q+2*TAU)/1e10
...
LOW=np.array([0.018,3.20,0.30,0.038])
HIGH=np.array([0.034,4.05,0.43,0.058])
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SN036BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
sam=qmc.Sobol(d=4,scramble=True,seed=360041)
...
def fit_gp(rows):
    ...
    gp=GaussianProcessRegressor(kernel=ker,normalize_y=True,n_restarts_optimizer=3,random_state=360042)
    gp.fit(Xn,y)
    return gp,float(y.min())

# Sequential expected-improvement Bayesian optimization.
rng=np.random.default_rng(360043)
for it in range(20):
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure107_bo002_sobol036_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"recover Planck on the coupled sobol036 background; exact raw DESI promotion decides whether Pantheon+ and Union3 close"}
(OUT/"snclosure107_bo002_sobol036_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SN036BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
