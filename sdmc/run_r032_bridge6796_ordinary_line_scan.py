#!/usr/bin/env python3
"""
Local 5D ordinary-sector Bayesian optimization for Planck recovery around bridge6796.

Frozen:
  H0 and late background = sobol107
  SDMC structure = bo002
...
OUT=Path("output/bridge6796_ordinary_line")
...
ALATE=0.011061627057101114
BLATE=0.010557478912554592
TAUA=0.25
TAUB=1.5
DNT=0.5
...
LOW=np.array([OB0-0.00070,OC0-0.0060,NS0-0.0150,Q0-0.0250,max(0.032,TAU0-0.0180)])
HIGH=np.array([OB0+0.00070,OC0+0.0060,NS0+0.0150,Q0+0.0250,min(0.085,TAU0+0.0180)])
...
LOCAL_SN=dict(pantheonplus=1403.8851876021363,union3=26.547333491500467,desy5=1645.4675567299128)
...
def finish(rec):
    ...
    rec["goal_score"]=float(dp + 500.0*max(0.0,rec["second_sn_delta"]))
    return rec

center=finish(center)
rows=[center]

# Deterministic ordinary-sector line scan from the bridge6796 point toward local021.
X1=np.array([
    LOCAL021["omega_b"],
    LOCAL021["omega_cdm"],
    LOCAL021["n_s"],
    math.log(1e10*LOCAL021["A_s"])-2*LOCAL021["tau"],
    LOCAL021["tau"]
],float)

rows=[center]
for i,t in enumerate(np.linspace(-0.20,1.40,33)):
    x=X0+t*(X1-X0)
    r=finish(evaluate(f"line{i:02d}",x))
    r["line_t"]=float(t)
    rows.append(r)
    print("ORDLINE_POINT",json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"bridge6796_ordinary_line_scan.csv",index=False)
ok=df[(df.status=="OK") & (df.stable_subluminal==True)].copy()
ok["two_sn_closed"]=ok[["sn_delta_pantheonplus","sn_delta_union3","sn_delta_desy5"]].apply(
    lambda r: sorted(map(float,r))[1] <= 0.0, axis=1)
ok["line_goal"]=ok["delta_planck_lite"] + 250.0*np.maximum(0.0,ok["second_sn_delta"])
top=ok.sort_values(["line_goal","delta_planck_lite","second_sn_delta"]).head(15)
top.to_csv(OUT/"bridge6796_ordinary_line_top15.csv",index=False)
summary=dict(
  target="recover Planck along ordinary-sector bridge6796->local021 line while retaining at least two SN <= matched LCDM",
  center_planck_lite=CPL,
  best=top.iloc[0].to_dict() if len(top) else {},
  best_two_sn=(ok[ok.two_sn_closed].sort_values("delta_planck_lite").head(1).to_dict("records")),
  top15=top.to_dict("records"),
  note="All points keep H0=67.96 and bridge6796 late/structural sector fixed; exact full Plik + raw DESI promotion required."
)
(OUT/"bridge6796_ordinary_line_summary.json").write_text(json.dumps(summary,indent=2))
print("ORDLINE_BEST",json.dumps(summary,sort_keys=True),flush=True)
