#!/usr/bin/env python3
"""
Bayesian structural recovery around the SN-aware sobol021 late background.

Low-z geometry is frozen at the SN-friendly point:
  A_late=0.02291857772739604
  B_late=0.012927192327566447
...
OUT=Path("output/snclosure021_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
AL=0.02291857772739604; BL=0.012927192327566447
...
EDGE_P_LITE=1024.4254805757748
EDGE_PD_FAIR=0.44702744377813985
EDGE_BAO=0.0
NEW_BAO=0.0
SN_PP=-0.04021468618884683
SN_U3=0.04800412361510098
SN_D5=0.03232041001319885

LOW=np.array([0.0225,3.55,0.285,0.040])
HIGH=np.array([0.040,4.35,0.425,0.060])
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SN021BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
sam=qmc.Sobol(d=4,scramble=True,seed=72041)
...
def fit_gp(rows):
    ...
    gp=GaussianProcessRegressor(kernel=ker,normalize_y=True,n_restarts_optimizer=3,random_state=72042)
    gp.fit(Xn,y)
    return gp,float(y.min())

# Sequential expected-improvement Bayesian optimization.
rng=np.random.default_rng(72043)
for it in range(20):
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure021_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"P+D fair < -0.032321 closes Pantheon+ and DES-Y5; < -0.048005 also closes Union3; BO optimizes Planck-lite while raw DESI is checked on promoted points"}
(OUT/"snclosure021_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SN021BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
