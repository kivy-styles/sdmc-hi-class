...
OUT=Path("output/wide_ridge"); OUT.mkdir(parents=True,exist_ok=True)
...
AF=0.02 + u[:,0]*(0.08-0.02)
ZC=0.8 + u[:,1]*(12.0-0.8)
W=np.exp(np.log(0.20)+u[:,2]*(np.log(3.0)-np.log(0.20)))
design=[("current",0.0715,5.0,1.426)]
...
for i,(tag,af,zc,w) in enumerate(design,1):
    ...
    print("WIDERIDGE_BG",json.dumps(rec,sort_keys=True),flush=True); rows.append(rec)

df=pd.DataFrame(rows); df.to_csv(OUT/"wide_ridge_stability.csv",index=False)
...
print("WIDERIDGE_CURRENT",cur.to_dict(),flush=True)
print("WIDERIDGE_LOWEST_F0",stable.nsmallest(30,"F0").to_dict("records"),flush=True)
...
keep=set(["current"])
keep.update(stable.nsmallest(24,"F0").id.tolist())
low=stable[stable.F0 < float(cur.F0)]
if not low.empty:
    keep.update(low.nlargest(12,"min_cs2").id.tolist())
...
for j,r in sel.iterrows():
    ...
    print("WIDERIDGE_CMB",json.dumps(rec,sort_keys=True,default=str),flush=True); scores.append(rec)

sd=pd.DataFrame(scores); sd.to_csv(OUT/"wide_ridge_planck.csv",index=False)
ok=sd[sd.cmb_status=="OK"]
print("WIDERIDGE_BEST_PLANCK",ok.nsmallest(20,"delta_planck").to_dict("records"),flush=True)
print("WIDERIDGE_BEST_LOW_F0",ok.nsmallest(20,"F0").to_dict("records"),flush=True)
