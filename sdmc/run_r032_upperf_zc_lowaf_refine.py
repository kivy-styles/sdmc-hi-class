#!/usr/bin/env python3
"""
Targeted continuation of the lower-z_t local basin toward higher f/z_c and lower A_F.
...
OUT=Path("output/r032_upperf_zc_lowaf_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.17,0.0468,2.93,0.435,0.0415,15.55],float)
HIGH=np.array([0.30,0.0486,3.035,0.490,0.0530,16.22],float)
...
def candidate(tag,f,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("UFZC_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
        ...
        if abs(gbest)>0.04:
            ...
            print("FRIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
        H0=Hbest
    else:
    ...
    print("FRIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
anchors=[
 ("lzlocal_best",0.21917187053710221,0.04818102984447032,2.9603643837291744,0.44650924081914123,0.04514786952827126,16.075284339487553),
 ("near2",0.17883776299655438,0.04765946148475632,2.967979272911325,0.47378863507416097,0.0465546010946855,15.7384651427716),
 ("near3",0.21274041024968027,0.04770209687678143,2.974019671594724,0.458951536109671,0.04980833407957107,15.872307556625456),
 ("highf_zc",0.255,0.04765,2.990,0.456,0.0475,15.92),
 ("lowaf_zc",0.225,0.04725,2.985,0.465,0.0485,15.84),
]
for a in anchors: rows.append(candidate(*a))

sam=qmc.Sobol(d=6,scramble=True,seed=20260926)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_upperf_zc_lowaf_refine.csv",index=False)
...
(OUT/"r032_upperf_zc_lowaf_refine_summary.json").write_text(json.dumps(summary,indent=2))
print("UFZC_BEST",json.dumps(best,sort_keys=True),flush=True)
print("UFZC_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
