#!/usr/bin/env python3
"""
Focused late-background joint-closure screen around exact bo008, which already beats LCDM local021 on combined Planck+DESI.
...
OUT=Path("output/bo008_jointclosure"); OUT.mkdir(parents=True,exist_ok=True)
TMP=OUT/"tmp"; TMP.mkdir(exist_ok=True)
SNROOT=Path("sn_data")
BAOROOT=Path("bao_data")

# Validated bo008 structural / primordial anchor.
OB=0.0221314201433272
OC=0.1231236773086558
AS=2.113173767730034e-9
NS=0.9621261195904712
TAU=0.0533406680635174
...
A0=0.0168678653798997
B0=0.0145431955903768
H00=70.00876109628007
EDGE024_PD_FAIR=-0.5306676969854236
CENTER_BAO_CHI=13.251270395581628
...
for i,x in enumerate(u):
    ...
    if r["status"]=="OK":
        ...
        r["stage1_score"]=sn2 + 1600*abs(r["ellA_frac"]) + 0.10*max(0,r["bao_chi2"]-CENTER_BAO_CHI)
    rows.append(r)
...
pdf["pd_proxy"]=EDGE024_PD_FAIR+pdf.delta_planck_vs_center+0.25*(pdf.bao_chi2-CENTER_BAO_CHI)
