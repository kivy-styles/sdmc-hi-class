#!/usr/bin/env python3
"""
Fine Q re-closure around the lowridge021 structural winner.

All geometry, matter, the lowridge021 SDMC structure, n_s and tau are fixed.
...
OUT=Path("output/lowridge021_q_reclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("LRQ_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"lowridge021_q_reclosure.csv",index=False)
...
(OUT/"lowridge021_q_reclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("LRQ_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LRQ_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
