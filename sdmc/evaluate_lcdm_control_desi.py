...
models=[
    ("LCDM_REFERENCE",model_data('lcdm_ref',67.36,0.02237,0.1200,0.9649)),
    ("LCDM_SOBOL054",model_data('lcdm_054',68.78113517636451,0.022678412813829034,0.11795410220236471,0.9621755753970704)),
    ("LCDM_SOBOL023",model_data('lcdm_023',67.60664539063684,0.022342536433760673,0.11976407214092616,0.9538520053924993)),
]
...
for label,model in models:
    rr,chi=profile_model(label,model)
    rows.extend(rr)
    summary.append(dict(model=label,chi2_profile=chi))
refchi=next(x["chi2_profile"] for x in summary if x["model"]=="LCDM_REFERENCE")
for x in summary:
    x["delta_vs_reference"]=x["chi2_profile"]-refchi
    print("LCDM_CONTROL_DESI_SUMMARY",x,flush=True)
pd.DataFrame(rows).to_csv(OUT/'lcdm_control_desi_profile.csv',index=False)
pd.DataFrame(summary).to_csv(OUT/'lcdm_control_desi_summary.csv',index=False)
