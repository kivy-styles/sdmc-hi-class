#!/usr/bin/env python3
"""
Bayesian structural recovery around the SN-aware sobol005 late background.

Low-z geometry is frozen at the SN-friendly point:
  A_late=0.02241910859523341
  B_late=0.01143003675621003
  H0=69.4601518549025
...
OUT=Path("output/snclosure005_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
AL=0.02241910859523341; BL=0.01143003675621003
...
EDGE_P_LITE=1025.033549588348
EDGE_PD_FAIR=1.0905489917928404
EDGE_BAO=0.0
NEW_BAO=0.0
SN_PP=-0.5202889665961266
SN_U3=-0.3864401327155065
SN_D5=-0.9510709717869759
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SN005BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure005_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"all three SN are already favorable; exact promotion must achieve P+D fair < 0; BO optimizes Planck-lite while raw DESI is checked on promoted points"}
(OUT/"snclosure005_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SN005BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
