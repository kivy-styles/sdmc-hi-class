...
OUT=Path("output/r032_localref008_af_extension"); OUT.mkdir(parents=True,exist_ok=True)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("AFEXT_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("AFEXT_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
print("AFEXT_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
