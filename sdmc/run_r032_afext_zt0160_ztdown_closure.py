#!/usr/bin/env python3
"""
Direct z_t closure around the AF-extension zt0160 winner.

Motivation:
  The AF-extension winner zt0160 improves the localref008 Planck-lite score
  and sits at z_t=16.10. Test whether the fit continues improving toward
  earlier z_t while holding the compensated structural shape fixed.
...
OUT=Path("output/r032_afext_zt0160_ztdown_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
ZT_VALUES=[15.80,15.85,15.90,15.95,16.00,16.025,16.05,16.075,16.10,16.125,16.15,16.20,16.25]
...
df.to_csv(OUT/"r032_afext_zt0160_ztdown_closure.csv",index=False)
...
summary={
 ...
 "best_at_lower_edge":bool(len(ok) and abs(float(ok.nsmallest(1,"chi2_planck").iloc[0].z_t)-min(ZT_VALUES))<1e-10)
}
(OUT/"r032_afext_zt0160_ztdown_closure_summary.json").write_text(json.dumps(summary,indent=2))
