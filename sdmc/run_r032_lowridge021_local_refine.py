...
OUT=Path("output/r032_lowridge021_local_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0350,3.05,0.345,0.0420,15.90],float)
HIGH=np.array([0.0455,3.85,0.465,0.0560,16.42],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LRREF_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("LRREF_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"r032_lowridge021_local_refine.csv",index=False)
...
(OUT/"r032_lowridge021_local_refine_summary.json").write_text(json.dumps(summary,indent=2))
print("LRREF_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LRREF_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
