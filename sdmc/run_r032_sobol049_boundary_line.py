...
OUT=Path("output/sobol049_boundary_line"); OUT.mkdir(parents=True,exist_ok=True)
...
print("BOUNDARY_LINE_LCDM",base,flush=True)

V009=np.array([70.79871074482799,0.02221026752647944,0.12381796840205787,
                 0.9633178803175687,0.0596811323207803,3.0596968970261513,
                 0.05435267057083547,3.1564399330876767,0.517175733782351],float)
V049=np.array([70.6661848725751,0.02213832986768335,0.12419066331610083,
                 0.9614044621046632,0.05464880801877007,3.051477308589965,
                 0.05709640212077647,2.8532181778922676,0.575869657304138],float)
TVALS=[-0.25,0.0,0.25,0.5,0.75,1.0,1.1,1.2,1.3,1.4,1.5,1.65,1.8,2.0]
design=[]
for t in TVALS:
    v=V009+t*(V049-V009)
    tag=("line_t"+f"{t:+.2f}").replace("+","p").replace("-","m").replace(".","p")
    design.append((tag,v))
print("BOUNDARY_LINE_DESIGN",[
    dict(id=tag,H0=float(v[0]),omega_b=float(v[1]),omega_cdm=float(v[2]),
         n_s=float(v[3]),tau=float(v[4]),lnAs=float(v[5]),AF=float(v[6]),
         zc=float(v[7]),width=float(v[8]))
    for tag,v in design],flush=True)
...
for i,(tag,v) in enumerate(design,1):
    ...
    rows.append(rec); print("BOUNDARY_LINE_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"sobol049_boundary_line.csv",index=False)
...
for col,label in [("delta_planck","PLANCK"),("joint_pp","PP"),("joint_union3","UNION3"),("joint_desy5","DESY5")]:
    print("BOUNDARY_LINE_BEST_"+label,ok.nsmallest(12,col).to_dict("records"),flush=True)
