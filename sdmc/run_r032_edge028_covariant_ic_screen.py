...
OUT=Path("output/edge028_covariant_ic_screen"); OUT.mkdir(parents=True,exist_ok=True)
...
# parameter[0] remains the automatic Omega_smg shooting variable.
# These two are independent IC perturbations.
PHI_SHIFTS=[-0.02,-0.01,-0.005,0.0,0.005,0.01,0.02]
VEL_SCALES=[0.98,0.99,0.995,1.0,1.005,1.01,1.02]
...
Dt=target["kin (D)"].to_numpy()[order]
Ct=target["c_s^2"].to_numpy()[order]

def ini(root,dphi,vscale):
    return f"""H0={H0:.17g}
omega_b={OB:.17g}
omega_cdm={OC:.17g}
N_ncdm=0
...
parameters_smg=0.0,{dphi:.17g},{vscale:.17g}
...
rows=[]
for i,dphi in enumerate(PHI_SHIFTS):
  for j,vscale in enumerate(VEL_SCALES):
    tag=f"ic_{i:02d}_{j:02d}"
    root=str(OUT/(tag+"_")); ip=OUT/(tag+".ini")
    ip.write_text(ini(root,float(dphi),float(vscale)))
    cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=360)
    rec={"id":tag,"phi_shift":float(dphi),"vel_scale":float(vscale),
         "returncode":int(cp.returncode),"status":"FAIL"}
    m=re.findall(r"found 'shooting_parameter_smg = ([^']+)'",cp.stdout)
    if m:
      try: rec["tuned_budget_parameter"]=float(m[-1])
      except: pass
    ...
    if cp.returncode==0 and bgp.exists() and thp.exists():
        ...
        Dref=np.interp(N,Nt,Dt); Cref=np.interp(N,Nt,Ct)
        H=bg["H [1/Mpc]"].to_numpy(); F=bg["M*^2_smg"].to_numpy()
        D=bg["kin (D)"].to_numpy(); C=bg["c_s^2"].to_numpy()
        hrel=np.abs(H/Href-1.); frel=np.abs(F/Fref-1.)
        drel=np.abs(D-Dref); crel=np.abs(C-Cref)
        ...
        rec.update(
          ...
          rms_dF_F_z100=float(np.sqrt(np.mean(frel[m100]**2))),
          max_abs_dD_z100=float(drel[m100].max()),
          rms_dD_z100=float(np.sqrt(np.mean(drel[m100]**2))),
          max_abs_dcs2_z100=float(crel[m100].max()),
          min_D=float(D.min()),min_cs2=float(C.min()),max_cs2=float(C.max()),
    ...
    print("IC_POINT",json.dumps(rec,sort_keys=True),flush=True)
    rows.append(rec)

df=pd.DataFrame(rows); df.to_csv(OUT/"edge028_covariant_ic_screen.csv",index=False)
...
summary={
 ...
 "best_D_replay":None if len(ok)==0 else ok.nsmallest(1,"rms_dD_z100").iloc[0].to_dict(),
}
(OUT/"edge028_covariant_ic_screen_summary.json").write_text(json.dumps(summary,indent=2,default=float))
print("IC_SUMMARY",json.dumps(summary,sort_keys=True,default=float),flush=True)
