...
OUT=Path("output/acoustic_physical_fullplik")
...
CANDIDATES=[
    dict(id="amp_best",tau=0.0544,lnAs=3.060),
    dict(id="amp_alt",tau=0.0524,lnAs=3.057),
    dict(id="amp_high_tau",tau=0.0564,lnAs=3.064),
]
...
for c in CANDIDATES:
    ...
    print("APHYS_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("APHYS_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("APHYS_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("APHYS_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"acoustic_physical_fullplik.csv",index=False)
...
(OUT/"acoustic_physical_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("APHYS_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
