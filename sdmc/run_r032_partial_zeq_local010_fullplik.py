...
OUT=Path("output/partial_zeq_local010_fullplik")
...
CANDIDATES=[
    dict(id="partial_zeq_local010",H0=70.69065645344024,ob=0.022063253393818805,
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("PZEQLOCAL010_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("PZEQLOCAL010_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("PZEQLOCAL010_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("PZEQLOCAL010_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("PZEQLOCAL010_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"partial_zeq_local010_fullplik.csv",index=False)
...
(OUT/"partial_zeq_local010_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQLOCAL010_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
