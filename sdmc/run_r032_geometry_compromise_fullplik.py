...
OUT=Path("output/geometry_compromise_fullplik")
...
CANDIDATES=[
    dict(id="t025_best",H0=70.5659273196888,ob=0.022329086486197414,
         oc=0.12479662145108897,ns=0.963943012708798,tau=0.0544,lnAs=3.060),
    dict(id="t0125_neighbor",H0=70.7086636598444,ob=0.022364303243098706,
         oc=0.12461944737013873,ns=0.963971506354399,tau=0.0544,lnAs=3.060),
    dict(id="t0375_neighbor",H0=70.4231909795332,ob=0.022293869729296117,
         oc=0.12497379553203922,ns=0.963914519063197,tau=0.0544,lnAs=3.060),
    dict(id="t025_lowAs",H0=70.5659273196888,ob=0.022329086486197414,
         oc=0.12479662145108897,ns=0.963943012708798,tau=0.0544,lnAs=3.056),
    dict(id="t025_highAs",H0=70.5659273196888,ob=0.022329086486197414,
         oc=0.12479662145108897,ns=0.963943012708798,tau=0.0544,lnAs=3.064),
]
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("GCFP_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("GCFP_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("GCFP_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("GCFP_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("GCFP_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"geometry_compromise_fullplik.csv",index=False)
...
(OUT/"geometry_compromise_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("GCFP_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
