#!/usr/bin/env python3
"""
High-A SN-aware late-background extension around the successful edge024-Q basin.
...
OUT=Path("output/snclosure_highA_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
print("SNHIGHA_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

sob=qmc.Sobol(d=3,scramble=True,seed=240924)
u=sob.random_base2(m=7)  # 128
# Broad but physically conservative low-z box.
Alo,Ahi=0.0350,0.0750
Blo,Bhi=0.0100,0.0220
Hlo,Hhi=69.50,70.40
rows=[]
for i,x in enumerate(u):
    ...
    rows.append(r)
    if i%16==0: print("SNHIGHA_PROGRESS",i,json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"snclosure_highA_stage1_all.csv",index=False)
...
short.to_csv(OUT/"snclosure_highA_stage1_shortlist.csv",index=False)
...
def planck_for(row):
    ...
    print("SNHIGHA_PLANCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
pdf.to_csv(OUT/"snclosure_highA_planck_shortlist.csv",index=False)
...
(OUT/"snclosure_highA_summary.json").write_text(json.dumps(summary,indent=2))
print("SNHIGHA_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNHIGHA_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
