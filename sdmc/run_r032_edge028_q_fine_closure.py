#!/usr/bin/env python3
"""
Fine Q closure around the exact-promoted edge028 structural solution.
...
OUT=Path("output/edge028_q_fine_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("EDGE_QFINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"edge028_q_fine_closure.csv",index=False)
...
(OUT/"edge028_q_fine_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("EDGE_QFINE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("EDGE_QFINE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
