#!/usr/bin/env python3
"""
Lower-A_F extension around the lrref008 structural screen winner.

Motivation:
  The lowridge021 local refinement found lrref008 with a 1.3078 chi2
  Planck-lite improvement, but its A_F=0.03599 lies close to that scan's
  lower A_F boundary of 0.035.  Extend A_F downward while allowing z_c,
  width, D_floor and z_t to move jointly, preserving the acoustic lock.
...
OUT=Path("output/r032_lrref008_lowaf_extension"); OUT.mkdir(parents=True,exist_ok=True)
...
AF0=0.035991880638059234
ZC0=3.7172300264239313
W0=0.4018391790613532
D0=0.34231919445927034
DF0=0.045995755137875675
LAM=18.40625
ZT0=16.24029149528593

# A_F, z_c, width, D_floor, z_t
LOW=np.array([0.0270,3.35,0.340,0.0400,15.85],float)
HIGH=np.array([0.0385,4.10,0.455,0.0600,16.40],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LRLOWAF_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("LRLOWAF_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec

rows=[]
base=candidate("lrref008",AF0,ZC0,W0,DF0,ZT0)
if base.get("status")!="OK": raise RuntimeError("lrref008 baseline failed")
BASE=float(base["chi2_planck"]); rows.append(base)

anchors=[
 ("af034",0.0340,3.72,0.400,0.0470,16.22),
 ("af032",0.0320,3.76,0.395,0.0490,16.18),
 ("af030",0.0300,3.82,0.390,0.0520,16.12),
 ("af028",0.0280,3.90,0.385,0.0550,16.05),
 ("af034zt016",0.0340,3.78,0.395,0.0480,16.16),
 ("af032zt010",0.0320,3.85,0.390,0.0500,16.10),
]
for a in anchors:
    rows.append(candidate(*a))

sam=qmc.Sobol(d=5,scramble=True,seed=20260925)
...
for r in rows:
    if r.get("status")=="OK":
        r["delta_planck_vs_lrref008"]=r["chi2_planck"]-BASE
        r["delta_high_vs_lrref008"]=r["chi2_high"]-base["chi2_high"]
        r["delta_lowT_vs_lrref008"]=r["chi2_lowT"]-base["chi2_lowT"]
        r["delta_lowE_vs_lrref008"]=r["chi2_lowE"]-base["chi2_lowE"]
        r["delta_lensing_vs_lrref008"]=r["chi2_lensing"]-base["chi2_lensing"]
        r["delta_cal_vs_lrref008"]=r["chi2_cal"]-base["chi2_cal"]

df=pd.DataFrame(rows)
df.to_csv(OUT/"r032_lrref008_lowaf_extension.csv",index=False)
...
summary={
 ...
 "best_delta_vs_lrref008":float(ok.chi2_planck.min()-BASE) if len(ok) else None,
}
(OUT/"r032_lrref008_lowaf_extension_summary.json").write_text(json.dumps(summary,indent=2))
print("LRLOWAF_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LRLOWAF_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
