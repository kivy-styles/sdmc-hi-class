...
OUT=Path("output/partial_zeq_best_desi")
...
MODELS={
    ...
    "PARTIAL_ZEQ_BEST":dict(
        kind="sdmc",
        H0=70.66324280546841,
        ob=0.022039097239759515,
        oc=0.12364444452954193,
        As=2.132477632355162e-9,
        ns=0.9625227132590487,
        tau=0.055202901571989066,
        AF=0.0590625,
        zc=2.775,
        width=0.5943750000000001,
        dfloor=0.04203125,
        lam=18.162499999999998,
        zt=17.1375,
    ),
}
...
print("PZEQBEST_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("PZEQBEST_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("PZEQBEST_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("PZEQBEST_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("PZEQBEST_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("PZEQBEST_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "PARTIAL_ZEQ_BEST":totals["PARTIAL_ZEQ_BEST"],
    "delta_partial_zeq_best_minus_lcdm":totals["PARTIAL_ZEQ_BEST"]-totals["LCDM"],
}
pd.DataFrame(allrows).to_csv(OUT/"partial_zeq_best_desi_profile.csv",index=False)
(OUT/"partial_zeq_best_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQBEST_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
