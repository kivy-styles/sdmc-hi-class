...
OUT=Path("output/partial_zeq_edge_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
rows=[]
rows.append(candidate("sobol016_center",0.06875,0.0590625,2.775,0.5943750000000001,0.04203125,18.1625,17.1375))
...
df=pd.DataFrame(rows); df.to_csv(OUT/"partial_zeq_edge_refine.csv",index=False)
...
(OUT/"partial_zeq_edge_refine_summary.json").write_text(json.dumps(summary,indent=2))
