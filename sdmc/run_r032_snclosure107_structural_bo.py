#!/usr/bin/env python3
"""
Bayesian structural recovery around the coupled sobol036 SN-friendly background.

Low-z geometry is frozen at the SN-friendly point:
  A_late=0.0013601897284388
  B_late=0.0147392870020121
  H0=69.80878993044607
...
OUT=Path("output/snclosure107_coupled036_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5

# Frozen SN-friendly background + ordinary sector
H0=69.80878993044607
OB=0.02208516937362783; OC=0.12298480378987971
NS=0.9625562783210716; TAU=0.055202901571989066
Q=math.log(2.119736350292785e-9*1e10)-2*TAU; AS=2.119736350292785e-9
AL=0.011249459300190211; BL=0.01280436261370778
...
EDGE_PD_FAIR=-0.33605484141273223
EDGE_BAO=13.204567845148436
NEW_BAO=13.204567845148436
SN_PP=0.5575426062569022
SN_U3=0.5355814192298567
SN_D5=1.2224958017468452

LOW=np.array([0.018,3.35,0.285,0.038])
HIGH=np.array([0.036,4.10,0.440,0.062])
...
def cand(tag,AF,ZC,W,DF):
    ...
    print("SNC036BO_POINT",json.dumps(r,sort_keys=True),flush=True)
    return r
...
sam=qmc.Sobol(d=4,scramble=True,seed=107036)
...
def fit_gp(rows):
    ...
    gp=GaussianProcessRegressor(kernel=ker,normalize_y=True,n_restarts_optimizer=3,random_state=107037)
    gp.fit(Xn,y)
    return gp,float(y.min())

# Sequential expected-improvement Bayesian optimization.
rng=np.random.default_rng(107038)
for it in range(20):
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclosure107_coupled036_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"recover Planck structure on coupled sobol036; exact P+D fair must be below -0.55754 to close Pantheon+ and Union3"}
(OUT/"snclosure107_coupled036_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
print("SNC036BO_BEST",json.dumps(summary,sort_keys=True),flush=True)
