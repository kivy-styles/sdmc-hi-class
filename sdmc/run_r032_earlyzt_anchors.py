#!/usr/bin/env python3
"""
Early-z_t correlated stability-ridge closure around the final r032 solution.

Motivation:
  The completed local ridge screen improved Planck by 0.258 chi2 and its
  winner landed at z_t=16.332678, essentially on the lower z_t edge of the
  search box. The stable sample also showed lower A_F, narrower width and
  earlier z_t tracking the downhill direction. Extend that correlated ridge
  to earlier z_t while raising D_floor when needed to preserve stability.
...
OUT=Path("output/r032_earlyzt_anchors"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0460,2.82,0.430,0.0430,15.90],float)
HIGH=np.array([0.0505,3.10,0.500,0.0650,16.38],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("EARLYZT_ANCHOR_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("EARLYZT_ANCHOR_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
# Explicit continuation from the completed ridge winner.
anchors=[
 ("ridge028",0.0489503458160907,2.9896906245313586,0.48081518225371833,0.0469228272177279,16.332678021918984),
 ("ezt025",0.0489,2.99,0.480,0.0480,16.25),
 ("ezt015",0.0485,3.00,0.470,0.0510,16.15),
 ("ezt005",0.0480,3.02,0.460,0.0550,16.05),
 ("eztm05",0.0475,3.05,0.450,0.0600,15.95),
]
for a in anchors:
    rows.append(candidate(*a))

# Anchor-only diagnostic: broad Sobol continuation runs on the sibling branch.
...
df.to_csv(OUT/"r032_earlyzt_anchors.csv",index=False)
...
(OUT/"r032_earlyzt_anchors_summary.json").write_text(json.dumps(summary,indent=2))
print("EARLYZT_ANCHOR_BEST",json.dumps(best,sort_keys=True),flush=True)
print("EARLYZT_ANCHOR_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
