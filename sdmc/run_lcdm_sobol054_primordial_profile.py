...
models=[("LCDM_REFERENCE",OUT/"lcdm_reference_00_cl_lensed.dat")]
for path in sorted(OUT.glob("lcdm_s54_*_00_cl_lensed.dat")):
    label=path.name.replace("_00_cl_lensed.dat","").upper()
...
for row in results[1:]:
    d=row["chi2_profile_total"]-ref["chi2_profile_total"]
    row["delta_vs_reference"]=d
    print("LCDM_S54_REFINE_DELTA",row["model"],d,flush=True)
fields=sorted({k for row in results for k in row})
with (OUT/"lcdm_sobol054_primordial_refine.csv").open("w",newline="") as f:
    w=csv.DictWriter(f,fieldnames=fields); w.writeheader(); w.writerows(results)
best=min(results[1:],key=lambda r:r["chi2_profile_total"])
print("LCDM_S54_REFINE_BEST",json.dumps(best,sort_keys=True),flush=True)
