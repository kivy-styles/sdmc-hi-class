#!/usr/bin/env python3
"""
Local refinement around the lowwidth021 r032 closure leader.

Motivation:
  The low-width screen found a stable -1.7404 Planck-screen improvement at
  A_F=0.0411132, z_c=3.431916, width=0.410488, D_floor=0.0478993,
  z_t=16.314355.  The stable sample shows higher D_floor is costly, while
  lower A_F and earlier z_t remain promising. Refine locally while allowing
  D_floor to approach the stability boundary from above.
...
OUT=Path("output/r032_lowwidth021_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0350,3.10,0.360,0.0400,15.95],float)
HIGH=np.array([0.0450,3.70,0.470,0.0520,16.38],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LW021REF_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("LW021REF_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
# Explicit local continuations from the lowwidth021 leader.
anchors=[
 ("lowwidth021",0.0411132003464736,3.431916184257716,0.4104880520515144,0.047899333223700526,16.314355247840286),
 ("ref1",0.0400,3.40,0.410,0.0460,16.20),
 ("ref2",0.0390,3.35,0.420,0.0450,16.10),
 ("ref3",0.0380,3.30,0.430,0.0440,16.00),
 ("ref4",0.0420,3.30,0.450,0.0440,16.20),
]
for a in anchors:
...
df.to_csv(OUT/"r032_lowwidth021_refine.csv",index=False)
...
(OUT/"r032_lowwidth021_refine_summary.json").write_text(json.dumps(summary,indent=2))
print("LW021REF_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LW021REF_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
