#!/usr/bin/env python3
"""
SDMC tri-likelihood multi-fidelity Bayesian search around the exact
sobol127 + seed015 leader.
...
OUT=Path("output/trilikelihood_mfbo3_sobol127"); OUT.mkdir(parents=True,exist_ok=True)
TMP=OUT/"tmp"; TMP.mkdir(exist_ok=True)
SNROOT=Path("sn_data"); BAOROOT=Path("bao_data")
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5

# Ordinary sector is reopened as the fourth BO direction.
...
def ordinary_at(t):
    return (OB0+t*(OB1-OB0), OC0+t*(OC1-OC0),
            NS0+t*(NS1-NS0), AS0+t*(AS1-AS0))
...
# Reference exact/cheap values at the exact sobol127 + seed015 leader.
A_REF=0.01264600746287033
B_REF=0.012972991137765347
H_REF=69.83619736097754
T_REF=0.05452757313847541
ZC_REF=3.4274108000472188
DF_REF=0.050407338682562114
PLITE_REF=1021.0852360745247
BAO_REF=13.215120771731913
EXACT_PD_REF=-0.21418646229219362
...
# Four-dimensional local search box around the exact leader:
# late A, late B, H0, and ordinary-sector displacement t_ord.
# The seed015 perturbation structure is frozen because its dedicated
# structural BO found only ~0.013 of further Planck-lite improvement.
LOW=np.array([0.0105,0.0118,69.74,0.000])
HIGH=np.array([0.0138,0.0135,69.92,0.095])
...
def ini_text(root,A,B,H0,TORD):
    OB,OC,NS,AS=ordinary_at(TORD)
...
parameters_smg={AF:.17g},{ZC_REF:.17g},{WIDTH:.17g},{D0:.17g},1.0,{DF_REF:.17g}
...
ANCHORS=[
 # A,B,H0,t_ord, Planck-lite, BAO, exact fair P+D.
 # All three exact anchors use the same seed015 perturbation structure.
 (A_REF,B_REF,H_REF,T_REF,PLITE_REF,BAO_REF,-0.21418646229219362),
 (0.011249459300190211,0.01280436261370778,69.80878993044607,
  0.014789129979908472,1021.2202619980889,13.204567845148436,0.05337920225869297),
 (0.012441244517453015,0.012150546975433827,69.68155793957413,
  0.039001916050910954,1021.4084486281279,13.241202617778763,0.6655402717911123),
]
def local_proxy(plite,bao):
    return EXACT_PD_REF+(plite-PLITE_REF)+0.25*(bao-BAO_REF)

Xa=[]; ya=[]
for A,B,H,T,pl,ba,ex in ANCHORS:
    Xa.append([A,B,H,T]); ya.append(ex-local_proxy(pl,ba))
Xa=np.asarray(Xa,float); ya=np.asarray(ya,float)
Xan=(Xa-LOW)/(HIGH-LOW)
corr_gp=GaussianProcessRegressor(
 kernel=ConstantKernel(1.0,(1e-3,10))*Matern(length_scale=np.ones(4)*0.7,length_scale_bounds=(0.08,5.0),nu=2.5)+WhiteKernel(0.01,(1e-6,0.3)),
 normalize_y=True,n_restarts_optimizer=4,random_state=312701)
corr_gp.fit(Xan,ya)

def evaluate(tag,x):
    A,B,H0,TORD=map(float,x)
    root=str(TMP/(tag+"_")); ip=TMP/(tag+".ini")
    ip.write_text(ini_text(root,A,B,H0,TORD))
    cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=300)
    r={"id":tag,"A":A,"B":B,"H0":H0,"t_ord":TORD,
       "ZC":ZC_REF,"D_floor":DF_REF,"status":"FAIL"}
    ...
    if cp.returncode==0 and bgp.exists() and clp.exists():
        ...
        if stable:
            ...
            xn=((np.array([[A,B,H0,TORD]])-LOW)/(HIGH-LOW))
...
# Initial design includes all three clean exact anchors plus a local Sobol design.
pts=[
 ("sobol127_exact",np.array([A_REF,B_REF,H_REF,T_REF])),
 ("seed015_exact",np.array([0.011249459300190211,0.01280436261370778,
                            69.80878993044607,0.014789129979908472])),
 ("sobol031_exact",np.array([0.012441244517453015,0.012150546975433827,
                            69.68155793957413,0.039001916050910954])),
]
sob=qmc.Sobol(d=4,scramble=True,seed=312702)
...
def fit_goal(rows):
    ...
    X=np.array([[r["A"],r["B"],r["H0"],r["t_ord"]] for r in ok],float)
    y=np.array([r["goal_pred"] for r in ok],float)
    Xn=(X-LOW)/(HIGH-LOW)
    gp=GaussianProcessRegressor(
      kernel=ConstantKernel(1.0,(1e-3,10))*Matern(length_scale=np.ones(4)*0.35,length_scale_bounds=(0.04,3.0),nu=2.5)+WhiteKernel(1e-5,(1e-8,0.1)),
      normalize_y=True,n_restarts_optimizer=4,random_state=312703)
    gp.fit(Xn,y)
    return gp,float(y.min())

rng=np.random.default_rng(312704)
for it in range(32):
    gp,ybest=fit_goal(rows)
    pool=rng.uniform(LOW,HIGH,size=(12000,4))
    ok=[r for r in rows if r.get("status")=="OK"]
    rb=min(ok,key=lambda r:r["goal_pred"])
    xb=np.array([rb["A"],rb["B"],rb["H0"],rb["t_ord"]])
    local=xb+rng.normal(size=(4000,4))*0.050*(HIGH-LOW)
    ...
    acq=(ei+0.045*corrstd)
    Xold=np.array([[r["A"],r["B"],r["H0"],r["t_ord"]] for r in rows],float)
...
df.to_csv(OUT/"trilikelihood_mfbo3_sobol127.csv",index=False)
ok=df[df.status.eq("OK")].sort_values(["goal_pred","second_joint_pred","pd_exact_pred"])
best=ok.head(12).to_dict("records")
summary={
 "search":"sobol127-centered tri-likelihood MFBO3 over A,B,H0,t_ord",
 ...
 "exact_reference":"sobol127+seed015",
 "n_ok":int(len(ok)),
 "best":best,
...
(OUT/"trilikelihood_mfbo3_sobol127_summary.json").write_text(json.dumps(summary,indent=2))
