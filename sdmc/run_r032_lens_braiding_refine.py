...
OUT=Path("output/lens_braiding_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
# Refine around the only stable/improving island from the coarse scan.
# Small negative amplitudes are retained to map the sign boundary explicitly.
design=[(0.0,1.5,0.8,"control")]
for A in [-0.006,-0.003,0.003,0.006,0.009,0.012,0.015,0.018,0.020,0.022,0.024,0.026]:
    for zl in [1.2,1.4,1.5,1.6,1.8]:
        for sl in [0.7,0.8,0.9,1.0]:
            design.append((A,zl,sl,"refine"))
...
for i,(A,zl,sl,kind) in enumerate(design,1):
    ...
    rows.append(rec); print("LENS_BRAID_REFINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
...
df.to_csv(OUT/"lens_braiding_refine.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
print("LENS_BRAID_REFINE_ZERO",zrow.to_dict(),flush=True)
print("LENS_BRAID_REFINE_BEST_TOTAL",ok.nsmallest(15,"chi2_total").to_dict("records"),flush=True)
print("LENS_BRAID_REFINE_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("LENS_BRAID_REFINE_BEST_CAL_LENS",ok.assign(cal_lens=ok.chi2_cal+ok.chi2_lensing).nsmallest(15,"cal_lens").to_dict("records"),flush=True)
