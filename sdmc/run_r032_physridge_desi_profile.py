...
models={
  'PHYSICAL_RIDGE':model_data('desi_physical',69.7095092787552,0.02211778594478965,0.1258596659367904,0.9637720508351921),
  'ACOUSTIC_ONLY':model_data('desi_acoustic',69.7095092787552,0.02211778594478965,0.1258596659367904,0.9637720508351921),
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print('PHYSRIDGE_DESI_TRIAL',label,b['namespace'],iseed,float(opti.fun),
        ...
        rows.append(row)
        print('PHYSRIDGE_DESI_BLOCK',json.dumps(row,sort_keys=True),flush=True)
    print('PHYSRIDGE_DESI_TOTAL',label,total,flush=True)
    return rows,total

allrows=[]; totals={}
for label,m in models.items():
    rows,chi=profile_model(label,m)
    allrows.extend(rows)
    totals[label]=chi

summary={
  'physical_ridge_chi2':totals['PHYSICAL_RIDGE'],
  'acoustic_chi2':totals['ACOUSTIC_ONLY'],
  'lcdm_chi2':totals['LCDM'],
  'delta_physical_ridge_minus_lcdm':totals['PHYSICAL_RIDGE']-totals['LCDM'],
  'delta_acoustic_minus_lcdm':totals['ACOUSTIC_ONLY']-totals['LCDM'],
  'delta_physical_ridge_minus_acoustic':totals['PHYSICAL_RIDGE']-totals['ACOUSTIC_ONLY'],
}
print('PHYSRIDGE_DESI_SUMMARY',json.dumps(summary,sort_keys=True),flush=True)
pd.DataFrame(allrows).to_csv(OUT/'physical_ridge_desi_dr1_fs_profile.csv',index=False)
pd.DataFrame([summary]).to_csv(OUT/'physical_ridge_desi_dr1_fs_summary.csv',index=False)
