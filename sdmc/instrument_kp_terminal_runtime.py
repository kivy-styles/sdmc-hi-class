...
if old not in s:
    raise SystemExit("coordinate diagnostic anchor not found")
...
if old_fk not in s:
    raise SystemExit("force diagnostic anchor not found")
s=s.replace(old_fk,new_fk,1)

old_call=r'''      sdmc_kp_terminal_terms(pba,pth,ppt,pvecback,R,k2,
                             delta_g,theta_g,theta_b,
                             &kp_C,&kp_dlnC,&kp_Fk);
      dy[pv->index_pt_theta_b] += kp_Fk;
      dy[pv->index_pt_theta_g] += kp_Fk;'''
new_call=r'''      sdmc_kp_terminal_terms(pba,pth,ppt,pvecback,R,k2,
                             delta_g,theta_g,theta_b,
                             &kp_C,&kp_dlnC,&kp_Fk);
      {
