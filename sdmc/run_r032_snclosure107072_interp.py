...
OUT=Path("output/snclosure107072_interp"); OUT.mkdir(parents=True,exist_ok=True)
...
print("SNINTERP_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

# Dense interpolation between the two most promising SN-aware late backgrounds.
# Endpoint 107: smaller SN penalty; endpoint 072: stronger CMB/DESI geometry.
P107=np.array([0.0013601897284388,0.0147392870020121,70.09917331933976])
P072=np.array([0.0134815868735313,0.0181835683900862,70.60292498096824])
rows=[]
idx=0
# Main line, with finer sampling around the expected compromise t~0.4-0.7.
ts=sorted(set(list(np.linspace(0,1,21))+list(np.linspace(.30,.75,19))))
# Small transverse A/B perturbations keep H0 on the interpolation line while
# checking whether the original Sobol geometry missed a better distance shape.
offsets=[(0.0,0.0),(-0.0015,0.0),(0.0015,0.0),(0.0,-0.0015),(0.0,0.0015)]
for t in ts:
    p=(1-t)*P107+t*P072
    for da,db in offsets:
        A=float(max(0,p[0]+da)); B=float(max(0,p[1]+db)); H0=float(p[2])
        r=run_background(f"interp{idx:03d}",A,B,H0)
        r["interp_t"]=float(t); r["dA"]=float(da); r["dB"]=float(db)
        if r["status"]=="OK":
        ...
        rows.append(r); idx+=1
        if idx%20==0: print("SNINTERP_PROGRESS",idx,json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"snclosure107072_stage1_all.csv",index=False)
...
short=guard.nsmallest(36,"stage1_score").copy()
...
short.to_csv(OUT/"snclosure107072_stage1_shortlist.csv",index=False)
...
def planck_for(row):
    ...
    print("SNINTERP_PLANCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
pdf.to_csv(OUT/"snclosure107072_planck_shortlist.csv",index=False)
...
(OUT/"snclosure107072_summary.json").write_text(json.dumps(summary,indent=2))
print("SNINTERP_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNINTERP_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
