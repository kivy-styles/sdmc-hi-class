#!/usr/bin/env python3
"""
Lower-width stability-ridge closure around the final r032 zt-best + Qfine-best point.

Motivation:
  The previous wide structural map found its best high-l shapes at much
  narrower transition widths (~0.35-0.40) and lower A_F, but those points
  suffered a large calibration/amplitude penalty before the final Q closure.
  Since Qfine now drives A_planck close to unity, revisit that lower-width
  structural family with the final matter/primordial solution and acoustic
  lock, while explicitly compensating stability with D_floor.
...
OUT=Path("output/r032_lowwidth_ridge_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0400,2.75,0.300,0.0420,16.30],float)
HIGH=np.array([0.0505,4.00,0.480,0.0700,16.70],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LOWRIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("LOWRIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
# Explicit low-width anchors motivated by the old wide-ridge high-l winners.
anchors=[
 ("w045",0.0485,3.00,0.45,0.046,16.50),
 ("w042",0.0475,3.10,0.42,0.050,16.50),
 ("w040",0.0465,3.20,0.40,0.054,16.50),
 ("w038",0.0455,3.35,0.38,0.058,16.48),
 ("w036",0.0445,3.45,0.36,0.062,16.46),
 ("oldshape",0.0445,3.40,0.355,0.060,16.46),
]
for a in anchors:
    rows.append(candidate(*a))

sam=qmc.Sobol(d=5,scramble=True,seed=20260923)
...
df.to_csv(OUT/"r032_lowwidth_ridge_closure.csv",index=False)
...
(OUT/"r032_lowwidth_ridge_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("LOWRIDGE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LOWRIDGE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
