...
OUT=Path("output/sobol049_local_hypercube"); OUT.mkdir(parents=True,exist_ok=True)
...
print("LOCAL_HYPERCUBE_LCDM",base,flush=True)

CENTER=np.array([
    70.6661848725751,0.02213832986768335,0.12419066331610083,
    0.9614044621046632,0.05464880801877007,3.051477308589965,
    0.05709640212077647,2.8532181778922676,0.575869657304138
],float)

# Expand deliberately through the lower-n_s/lower-omega_b/lower-amplitude side
# that the original 64-point box could not explore, while staying local enough
# that DESI geometry/growth should remain recognizable.
LOW=np.array([70.45,0.02198,0.12380,0.9588,0.0505,3.042,0.0535,2.55,0.50],float)
HIGH=np.array([70.86,0.02230,0.12475,0.9638,0.0585,3.060,0.0615,3.12,0.67],float)

V075=np.array([70.69931634063832,0.02215631428238237,0.12409748958759009,
               0.9618828166578897,0.055906889094272626,3.0535322056990113,
               0.05641046923329122,2.92902361669112,0.5611961764236912],float)
V110=np.array([70.6529322853498,0.02213113610180374,0.12422793280750513,
               0.9612131202833727,0.05414557558856904,3.0506553497463464,
               0.05737077527577057,2.8228960023727265,0.5817390496563166],float)

anchors=[("sobol049_exact_center",CENTER),("line_t075",V075),("line_t110",V110)]
sob=qmc.Sobol(d=9,scramble=True,seed=202609211)
design=anchors+[(f"local{i+1:03d}",v) for i,v in enumerate(qmc.scale(sob.random_base2(m=6),LOW,HIGH))]
print("LOCAL_HYPERCUBE_DESIGN_SIZE",len(design),flush=True)
print("LOCAL_HYPERCUBE_BOUNDS",json.dumps({"low":LOW.tolist(),"high":HIGH.tolist()}),flush=True)
...
for i,(tag,v) in enumerate(design,1):
    ...
    rows.append(rec); print("LOCAL_HYPERCUBE_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"sobol049_local_hypercube.csv",index=False)
...
for col,label in [("delta_planck","PLANCK"),("joint_pp","PP"),("joint_union3","UNION3"),("joint_desy5","DESY5")]:
    print("LOCAL_HYPERCUBE_BEST_"+label,ok.nsmallest(12,col).to_dict("records"),flush=True)
