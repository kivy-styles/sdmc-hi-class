#!/usr/bin/env python3
"""
Edge extension around the lowridge021 final-Q stability-ridge winner.

Motivation:
  The low-width Sobol screen found lowridge021 with a 1.7404 chi2 Planck-lite
  improvement, but that winner lies close to the lower A_F and z_t boundaries.
  Extend both directions while locally re-optimizing z_c, width and D_floor.
...
OUT=Path("output/r032_lowridge_edge_extension"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0340,3.10,0.355,0.0430,15.90],float)
HIGH=np.array([0.0440,3.85,0.455,0.0660,16.34],float)
...
# Explicit extensions along the two open edges and their correlated ridge.
anchors=[
 ("leader",AF0,ZC0,W0,DF0,ZT0),
 ("zt1625",0.0411,3.43,0.4105,0.0480,16.25),
 ("zt1615",0.0410,3.43,0.4105,0.0485,16.15),
 ("zt1605",0.0408,3.45,0.4080,0.0490,16.05),
 ("af0400",0.0400,3.45,0.4050,0.0490,16.25),
 ("af0385",0.0385,3.50,0.4000,0.0510,16.18),
 ("af0370",0.0370,3.55,0.3950,0.0540,16.10),
]
for a in anchors:
...
df.to_csv(OUT/"r032_lowridge_edge_extension.csv",index=False)
...
(OUT/"r032_lowridge_edge_extension_summary.json").write_text(json.dumps(summary,indent=2))
