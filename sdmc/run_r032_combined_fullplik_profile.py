...
def run_one(label,path,seeds):
    ...
    for iseed,seed in enumerate(seeds):
        ...
        print("COMBINED_FULLPLIK_SEED",label,iseed,*[row[0],row[2],row[3]],flush=True)
    ...
    print("COMBINED_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x

models=[
  ("COMBINED",OUT/"combined_best_00_cl_lensed.dat"),
...
for a,b in [("COMBINED","LCDM"),("ACOUSTIC_ONLY","LCDM"),("COMBINED","ACOUSTIC_ONLY")]:
    d=r[a]["chi2_profile_total"]-r[b]["chi2_profile_total"]
    print("COMBINED_FULLPLIK_DELTA",a,"MINUS",b,d,flush=True)

fields=list(results[0].keys())
with (OUT/"combined_fullplik_profile.csv").open("w",newline="") as f:
