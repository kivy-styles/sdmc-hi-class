#!/usr/bin/env python3
"""
Focused local refinement around the lower-z_t / narrow-width winner.
...
OUT=Path("output/r032_lowerzt_best_local"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.135,0.0475,2.86,0.420,0.0390,15.60],float)
HIGH=np.array([0.235,0.0496,3.01,0.475,0.0500,16.18],float)
...
def candidate(tag,f,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LZLOCAL_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
...
anchors=[
 ("lowerzt_best",0.1767996006645262,0.04855956782167777,2.9370467670261857,0.448451985148713,0.044598941650241614,15.92094821177423),
 ("dfdown",0.1768,0.04856,2.937,0.4485,0.0420,15.92),
 ("afdown",0.1768,0.04810,2.937,0.4485,0.0455,15.92),
 ("wdown",0.1768,0.04856,2.937,0.4350,0.0455,15.90),
 ("mix",0.195,0.04830,2.920,0.4380,0.0440,15.82),
]
for a in anchors: rows.append(candidate(*a))

sam=qmc.Sobol(d=6,scramble=True,seed=20260925)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_lowerzt_best_local.csv",index=False)
...
(OUT/"r032_lowerzt_best_local_summary.json").write_text(json.dumps(summary,indent=2))
print("LZLOCAL_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LZLOCAL_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
