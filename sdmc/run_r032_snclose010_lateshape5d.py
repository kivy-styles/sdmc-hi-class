#!/usr/bin/env python3
"""
Five-dimensional late-shape SN-aware refinement around exact SN-close010.

Stage 1: 256 Sobol backgrounds in (A_late, B_late, tau_A, tau_B, H0), exact SN covariances,
...
OUT=Path("output/snclose010_lateshape5d"); OUT.mkdir(parents=True,exist_ok=True)
...
A0=0.062007905058562754
B0=0.018307528030127286
TAUA_C=0.25
TAUB_C=1.5
H00=70.31907490585
BASE_PD_FAIR=0.115040671971027
BASE_BAO_CHI=14.829618449898636
...
def ini_text(root,A,B,tauA,tauB,H0,cls=False):
...
def run_background(label,A,B,tauA,tauB,H0):
    root=str(TMP/(label+"_")); ip=TMP/(label+".ini")
    ip.write_text(ini_text(root,A,B,tauA,tauB,H0,False))
    cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,
                      text=True,timeout=120)
    rec={"id":label,"A":float(A),"B":float(B),"tauA":float(tauA),"tauB":float(tauB),"H0":float(H0),
...
center=run_background("center",A0,B0,TAUA_C,TAUB_C,H00)
if center["status"]!="OK": raise RuntimeError(f"center failed: {center}")
ELL0=center["ellA_proxy"]
print("SNLS5_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

sob=qmc.Sobol(d=5,scramble=True,seed=251024)
u=sob.random_base2(m=7)  # 128
# Broad but physically conservative low-z box.
Alo,Ahi=0.0450,0.1050
Blo,Bhi=0.0140,0.0240
TAlo,TAhi=0.12,0.38
TBlo,TBhi=0.9,2.2
Hlo,Hhi=70.15,70.75
rows=[]
for i,x in enumerate(u):
    A=Alo+(Ahi-Alo)*x[0]
    B=Blo+(Bhi-Blo)*x[1]
    tauA=TAlo+(TAhi-TAlo)*x[2]
    tauB=TBlo+(TBhi-TBlo)*x[3]
    H0=Hlo+(Hhi-Hlo)*x[4]
    r=run_background(f"sobol{i:03d}",A,B,tauA,tauB,H0)
    if r["status"]=="OK":
        r["ellA_frac"]=(r["ellA_proxy"]/ELL0-1.0)
        # If P+D did not move, these are the exact two easiest joint gaps.
        r["joint0_pp"]=BASE_PD_FAIR+r["sn_delta_pantheonplus"]
        r["joint0_u3"]=BASE_PD_FAIR+r["sn_delta_union3"]
        r["joint0_d5"]=BASE_PD_FAIR+r["sn_delta_desy5"]
        sn2=sorted([r["joint0_pp"],r["joint0_u3"],r["joint0_d5"]])[1]
        # Rank SN closure, while strongly preferring acoustic and BAO continuity.
        r["stage1_score"]=sn2 + 1600*abs(r["ellA_frac"]) + 0.10*max(0,r["bao_chi2"]-BASE_BAO_CHI)
    rows.append(r)
    if i%16==0: print("SNLS5_PROGRESS",i,json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"snclose010_lateshape5d_stage1_all.csv",index=False)
...
centerrow=pd.DataFrame([{**center,"ellA_frac":0.0,
                         ...
                         "joint0_pp":BASE_PD_FAIR+center["sn_pantheonplus"]-LOCAL_SN["pantheonplus"],
                         "joint0_u3":BASE_PD_FAIR+center["sn_union3"]-LOCAL_SN["union3"],
                         "joint0_d5":BASE_PD_FAIR+center["sn_desy5"]-LOCAL_SN["desy5"],
                         "stage1_score":999.}])
short=pd.concat([centerrow,short],ignore_index=True)
short.to_csv(OUT/"snclose010_lateshape5d_stage1_shortlist.csv",index=False)
...
def planck_for(row):
    ...
    ip.write_text(ini_text(root,float(row.A),float(row.B),float(row.tauA),float(row.tauB),float(row.H0),True))
    ...
    print("SNLS5_PLANCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
pdf["pd_proxy"]=BASE_PD_FAIR+pdf.delta_planck_vs_center+0.25*(pdf.bao_chi2-BASE_BAO_CHI)
...
pdf.to_csv(OUT/"snclose010_lateshape5d_planck_shortlist.csv",index=False)
best=pdf.head(8).to_dict("records")
summary={
 "base_pd_fair":BASE_PD_FAIR,
...
(OUT/"snclose010_lateshape5d_summary.json").write_text(json.dumps(summary,indent=2))
print("SNLS5_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNLS5_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
