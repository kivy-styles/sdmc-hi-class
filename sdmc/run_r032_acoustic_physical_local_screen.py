...
OUT=Path("output/acoustic_physical_local"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=0.0025
AF=0.0573794336710125; ZC=3.0902165911160413; WIDTH=0.5979282500222326
D0=0.34231919445927034; POWER=1.0; DFLOOR=0.045
LAMBDA_E=17.925; ZT=17.775; OMEGA_R_PHYS=4.17998772e-5

CURRENT=dict(H0=69.7095092787552,ob=0.02211778594478965,oc=0.1258596659367904,
             ns=0.9637720508351921,lnAs=3.0773358772153037,tau=0.06456347454525531)
...
print("ACOUSTIC_PHYS_LOCAL_LCDM",LCDM,flush=True)
...
LOW=np.array([0.02195,0.1235,0.960,0.054,Q0-0.010,-0.06])
HIGH=np.array([0.02240,0.1280,0.969,0.066,Q0+0.008,+0.06])
...
anchors=[
 ...
 ("tau060",np.array([CURRENT["ob"],CURRENT["oc"],CURRENT["ns"],0.060,Q0,dcur])),
 ("tau058_ns965",np.array([CURRENT["ob"],CURRENT["oc"],0.965,0.058,Q0,dcur])),
 ("tau056_ns966",np.array([CURRENT["ob"],CURRENT["oc"],0.966,0.056,Q0,dcur])),
 ("tau060_ns966_qm004",np.array([CURRENT["ob"],CURRENT["oc"],0.966,0.060,Q0-0.004,dcur])),
]
...
for i,(tag,x) in enumerate(design,1):
    ...
    if H0 is None:
        rec["error"]="no acoustic root"; print("ACOUSTIC_PHYS_LOCAL_POINT",json.dumps(rec),flush=True); rows.append(rec); continue
    ...
    print("ACOUSTIC_PHYS_LOCAL_POINT",json.dumps(rec,sort_keys=True),flush=True); rows.append(rec)
...
keep=set(["current","tau060","tau058_ns965","tau056_ns966","tau060_ns966_qm004"])
...
print("ACOUSTIC_PHYS_LOCAL_BEST_PLANCK",ok.nsmallest(12,"delta_planck").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LOCAL_BEST_PP",ok.nsmallest(12,"joint_pp").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LOCAL_BEST_UNION3",ok.nsmallest(12,"joint_union3").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LOCAL_BEST_DESY5",ok.nsmallest(12,"joint_desy5").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LOCAL_BEST_LENS",ok.nsmallest(12,"chi2_lensing").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LOCAL_BEST_CAL",ok.nsmallest(12,"chi2_cal").to_dict("records"),flush=True)
