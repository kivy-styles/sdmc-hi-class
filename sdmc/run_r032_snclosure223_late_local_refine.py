#!/usr/bin/env python3
"""
Local SN-aware late-background refinement around the sobol223 compromise basin.
...
OUT=Path("output/snclosure223_late_local_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
df.to_csv(OUT/"snclosure223_late_stage1_all.csv",index=False)
...
short.to_csv(OUT/"snclosure223_late_stage1_shortlist.csv",index=False)
...
pdf["pd_proxy"]=EDGE024_PD_FAIR+BASE_PLANCK_PENALTY+pdf.delta_planck_vs_center+0.25*(pdf.bao_chi2-EDGE024_BAO_CHI)
...
(OUT/"snclosure223_late_summary.json").write_text(json.dumps(summary,indent=2))
