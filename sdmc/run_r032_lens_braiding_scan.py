...
OUT=Path("output/lens_braiding"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=0.0025
H0=70.8514; OB=0.02239952; OC=0.12444227328918850
NS=0.964; TAU=0.0544; LNAS=3.076; AS=math.exp(LNAS)/1e10
...
# Exact zero-amplitude control is the baseline for deltas.
design=[]
amps=[-.05,-.04,-.03,-.0294,-.02,-.01,0.,.01,.02,.03,.04,.05]
for A in amps: design.append((A,2.2,.5,"amp"))
for A in [-.04,-.03,-.02,.02,.03,.04]:
    for zl in [1.5,2.2,3.0]:
        for sl in [.35,.8]:
            design.append((A,zl,sl,"loc"))
# de-duplicate exact tuples
seen=set(); dd=[]
for x in design:
    key=x[:3]
    if key not in seen: seen.add(key); dd.append(x)
design=dd
...
for i,(A,zl,sl,kind) in enumerate(design,1):
    ...
    if cp.returncode==0 and clp.exists() and pkp.exists():
        ...
        if stable:
            ...
            if abs(A)<1e-14 and zl==2.2 and sl==.5:
    ...
    rows.append(rec); print("LENS_BRAID_POINT",json.dumps(rec,sort_keys=True),flush=True)

# second pass fills deltas for points preceding the zero control
df=pd.DataFrame(rows)
zero=df[(df.A_lens.abs()<1e-14)&(df.z_lens==2.2)&(df.sigma_lens==.5)]
if zero.empty: raise SystemExit("zero control missing")
zrow=zero.iloc[0]
for k in ["chi2_high","chi2_lowT","chi2_lowE","chi2_lensing","chi2_cal","chi2_total"]:
    df["delta_"+k+"_vs_zero"]=df[k]-zrow[k]
df.to_csv(OUT/"lens_braiding_scan.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
print("LENS_BRAID_ZERO",zrow.to_dict(),flush=True)
print("LENS_BRAID_BEST_TOTAL",ok.nsmallest(15,"chi2_total").to_dict("records"),flush=True)
print("LENS_BRAID_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("LENS_BRAID_BEST_CAL_LENS",ok.assign(cal_lens=ok.chi2_cal+ok.chi2_lensing).nsmallest(15,"cal_lens").to_dict("records"),flush=True)
