#!/usr/bin/env python3
"""
Fine Q re-closure around the edge028 structural winner.

All geometry, matter, the edge028 SDMC structure, n_s and tau are fixed.
...
OUT=Path("output/edge028_q_reclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("EDGEQ_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
ok["delta_vs_edge028"]=ok.chi2_planck-center
df=pd.DataFrame(rows).merge(ok[["id","delta_vs_edge028"]],on="id",how="left")
df.to_csv(OUT/"edge028_q_reclosure.csv",index=False)
best=ok.nsmallest(8,"chi2_planck").to_dict("records")
summary={"Q0":Q0,"center_chi2":center,"best":best[0],
         "best_delta_vs_edge028":float(ok.chi2_planck.min()-center),
...
(OUT/"edge028_q_reclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("EDGEQ_BEST",json.dumps(best,sort_keys=True),flush=True)
print("EDGEQ_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
