#!/usr/bin/env python3
"""
Targeted simultaneous-compromise scan on the physical No-Slip ridge.

Interpolate the ordinary cosmology between:
  t=0 : frozen Part-VIII ordinary coordinates
  t=1 : acoustic-screen ordinary coordinates
while holding the best physical No-Slip Planck-mass ridge fixed and profiling
the primordial amplitude on a small deterministic grid.

The purpose is to find whether an intermediate geometry keeps the old point's
excellent high-l shape while retaining the acoustic point's near-zero
A_planck calibration penalty. SN compilations are evaluated separately.
"""
...
OUT=Path("output/physical_geometry_compromise"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025
OLD=np.array([70.8514,0.02239952,0.12444227328918850,0.964],float)
AC=np.array([69.7095092787552,0.02211778594478965,0.1258596659367904,0.9637720508351921],float)
TAU=0.0544
TS=np.linspace(0.,1.,9)
LNAS=[3.056,3.060,3.064,3.068,3.072,3.076]
...
def ini(root,x,lnAs):
    H0,ob,oc,ns=map(float,x); h=H0/100.; OX=1.-(ob+oc+OR)/(h*h)
    return textwrap.dedent(f"""\
    H0 = {H0:.15g}
    omega_b = {ob:.15g}
    omega_cdm = {oc:.15g}
...
for it,t in enumerate(TS):
    x=OLD+t*(AC-OLD)
    for lnAs in LNAS:
        tag=f"t{t:.3f}_a{lnAs:.3f}".replace(".","p")
        root=str(OUT/(tag+"_")); ip=OUT/(tag+".ini"); ip.write_text(ini(root,x,lnAs))
        cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=240)
        rec=dict(id=tag,t=float(t),H0=float(x[0]),omega_b=float(x[1]),omega_cdm=float(x[2]),
                 n_s=float(x[3]),tau_reio=TAU,ln10As=float(lnAs),returncode=cp.returncode,status="FAIL")
        ...
        rows.append(rec); print("PGC_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"physical_geometry_compromise.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
print("PGC_BEST_PLANCK",ok.nsmallest(15,"delta_planck").to_dict("records"),flush=True)
print("PGC_BEST_PP",ok.nsmallest(15,"joint_pp").to_dict("records"),flush=True)
print("PGC_BEST_UNION3",ok.nsmallest(15,"joint_union3").to_dict("records"),flush=True)
print("PGC_BEST_DESY5",ok.nsmallest(15,"joint_desy5").to_dict("records"),flush=True)
