...
OUT=Path("output/local002_fullplik")
...
CANDIDATES=[
    dict(id="local002",H0=70.5653567390982,ob=0.022011983189284802,
         oc=0.12404331885203719,ns=0.9625227132590487,tau=0.055202901571989066,
         lnAs=3.0598696043919773,AF=0.06017362505197525,
         zc=2.827464461401105,width=0.5514493708219379),
]
def table(path):
    import re
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("LOCAL002_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("LOCAL002_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("LOCAL002_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("LOCAL002_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("LOCAL002_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"local002_fullplik.csv",index=False)
...
(OUT/"local002_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("LOCAL002_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
