#!/usr/bin/env python3
"""
High-H0 SN-aware late-background extension around the successful edge024-Q basin.
...
OUT=Path("output/snclosure_highH0_refine"); OUT.mkdir(parents=True,exist_ok=True)
...
print("SNHIGHH0_CENTER_BACKGROUND",json.dumps(center,sort_keys=True),flush=True)

sob=qmc.Sobol(d=3,scramble=True,seed=240924)
u=sob.random_base2(m=7)  # 128
# Broad but physically conservative low-z box.
Alo,Ahi=0.0550,0.1400
Blo,Bhi=0.0140,0.0300
Hlo,Hhi=70.25,70.95
rows=[]
for i,x in enumerate(u):
    ...
    rows.append(r)
    if i%16==0: print("SNHIGHH0_PROGRESS",i,json.dumps(r,sort_keys=True),flush=True)

df=pd.DataFrame(rows)
df.to_csv(OUT/"snclosure_highH0_stage1_all.csv",index=False)
...
short.to_csv(OUT/"snclosure_highH0_stage1_shortlist.csv",index=False)
...
def planck_for(row):
    ...
    print("SNHIGHH0_PLANCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
pdf.to_csv(OUT/"snclosure_highH0_planck_shortlist.csv",index=False)
...
(OUT/"snclosure_highH0_summary.json").write_text(json.dumps(summary,indent=2))
print("SNHIGHH0_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNHIGHH0_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
