#!/usr/bin/env python3
"""
SN-aware ordinary-parameter refinement around SN-close010.

Fix the successful edge024-Q covariant structure and SN-friendly late
background shape found by the first SN-aware screen:
  A_late=0.018589897081255913
  B_late=0.013815921754576268
Reopen omega_b, omega_cdm, n_s, Q.  Solve H0 at each point to the same
acoustic scale, evaluate Planck native-lite and all three SN covariance
likelihoods, and rank candidates by the two-SN closure objective.

This remains a screen.  Any winner is promoted to exact covariant full Plik,
raw DESI full shape, and all three SN likelihoods.
"""
...
OUT=Path("output/snclose010_o4d_refine"); OUT.mkdir(parents=True,exist_ok=True)
SNROOT=Path("sn_data")
...
LOW=np.array([0.02190,0.1215,0.9595,2.9395])
HIGH=np.array([0.02230,0.1245,0.9660,2.9455])
...
m=ppz>0.01; ppm,ppz,ppzh,C=ppm[m],ppz[m],ppzh[m],C[np.ix_(m,m)]; ppp=setup(C)
...
def run_class(tag,H0,ob,oc,ns,Q,full):
    ...
    return root,bg,der,st

def candidate(tag,ob,oc,ns,Q):
    vals=[]
    for H in np.linspace(H00-0.9,H00+0.9,9):
        try:
            _,_,d,st=run_class(f"{tag}_r_{H:.4f}".replace(".","p"),H,ob,oc,ns,Q,False)
            if st["min_D"]>0 and st["min_cs2"]>0 and st["max_cs2"]<=1: vals.append((H,d["ell_A"]-TARGET))
        except: pass
    ...
    if pair is None:
        ...
        print("SN010O4D_POINT",json.dumps(rec,sort_keys=True),flush=True);return rec
    (h1,g1),(h2,g2)=pair; H0=float(h1-g1*(h2-h1)/(g2-g1))
    try: root,bg,der,st=run_class(tag,H0,ob,oc,ns,Q,True)
    except Exception as e:
        rec=dict(id=tag,status="FAIL",error=str(e)[-500:]); print("SN010O4D_POINT",json.dumps(rec,sort_keys=True),flush=True);return rec
    ...
    if stable:
        ...
        rec.update({f"sn_delta_{k}":v-LOCAL_SN[k] for k,v in sn.items()})
    print("SN010O4D_POINT",json.dumps(rec,sort_keys=True),flush=True);return rec

rows=[]
base=candidate("center",OB0,OC0,NS0,Q0); rows.append(base)
...
sam=qmc.Sobol(d=4,scramble=True,seed=22304)
...
for r in rows:
    if r.get("status")=="OK":
        ...
        # Ranking target: regain Planck while preserving two best SN penalties.
        sn2=sorted([r["sn_delta_pantheonplus"],r["sn_delta_union3"],r["sn_delta_desy5"]])[1]
        r["second_best_sn_delta"]=sn2
        r["score"]=r["delta_planck_vs_center"]+sn2

df=pd.DataFrame(rows); df.to_csv(OUT/"snclose010_o4d_refine.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
best=ok.nsmallest(12,["score","delta_planck_vs_center"]).to_dict("records")
summary={"baseline":base,"n_total":int(len(df)),"n_stable":int(len(ok)),
         "best":best[0] if best else None,"top":best}
(OUT/"snclose010_o4d_refine_summary.json").write_text(json.dumps(summary,indent=2))
print("SN010O4D_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SN010O4D_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
