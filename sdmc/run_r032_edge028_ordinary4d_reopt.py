#!/usr/bin/env python3
"""
Acoustically locked ordinary-parameter re-optimization around edge028.

The structural sector is fixed at edge028.  Re-open:
...
OUT=Path("output/r032_edge028_ordinary4d_reopt"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.02190,0.1216,0.9600,2.9405],float)
HIGH=np.array([0.02228,0.1244,0.9655,2.9455],float)
...
sam=qmc.Sobol(d=4,scramble=True,seed=20260926)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_edge028_ordinary4d_reopt.csv",index=False)
...
(OUT/"r032_edge028_ordinary4d_reopt_summary.json").write_text(json.dumps(summary,indent=2))
