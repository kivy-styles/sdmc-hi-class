#!/usr/bin/env python3
"""
Acoustic-cosmology plus physical No-Slip lensing-ridge scan.
...
OUT=Path("output/acoustic_physical_lensing"); OUT.mkdir(parents=True,exist_ok=True)
...
print("ACOUSTIC_PHYS_LENS_LCDM",base,flush=True)
...
anchors=[
    ("acoustic_current",(0.0715,5.0,1.426)),
    ("phys_best1",(0.0661676420550793,3.0664408076554537,0.5638187950477004)),
    ("phys_best2",(0.0642812754958868,3.251182517968118,0.6537219783756882)),
    ("phys_best3",(0.0746367159299552,2.405197636224329,0.6685318081174045)),
    ("phys_best4",(0.0505196195468306,7.780592767521739,0.4906709931790828)),
    ("phys_best5",(0.0572812970634549,5.567294009961188,0.7335329413879663)),
]
LOW=np.array([0.055,1.8,0.45])
HIGH=np.array([0.080,4.5,1.05])
...
for i,(tag,(AF,zc,width)) in enumerate(design,1):
    ...
    rows.append(rec)
    print("ACOUSTIC_PHYS_LENS_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"acoustic_physical_lensing_ridge.csv",index=False)
...
keep=set(["acoustic_current","phys_best1","phys_best2","phys_best3","phys_best4","phys_best5"])
...
short.to_csv(OUT/"acoustic_physical_lensing_shortlist.csv",index=False)

print("ACOUSTIC_PHYS_LENS_BEST_TOTAL",ok.nsmallest(15,"chi2_total").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LENS_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LENS_BEST_PP",ok.nsmallest(15,"pp_mismatch").to_dict("records"),flush=True)
print("ACOUSTIC_PHYS_LENS_SHORTLIST",short.to_dict("records"),flush=True)
