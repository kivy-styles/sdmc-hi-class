#!/usr/bin/env python3
"""
Fine Q re-closure around the SN-friendly sobol223 late-background winner.

All geometry, matter, the edge024 SDMC structure, n_s and tau are fixed.
...
OUT=Path("output/snclosure223_q_reclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
OFFSETS=np.array([-0.0050,-0.0045,-0.0040,-0.0035,-0.0030,-0.0025,-0.0020,-0.0015,-0.0010,-0.0005,\n                   0.0,0.0005,0.0010,0.0015,0.0020,0.0025,0.0030,0.0035,0.0040,0.0045,0.0050])
...
def run(i,dq):
    ...
    print("SNC223Q_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
ok["delta_vs_center"]=ok.chi2_planck-center
df=pd.DataFrame(rows).merge(ok[["id","delta_vs_center"]],on="id",how="left")
df.to_csv(OUT/"snclosure223_q_reclosure.csv",index=False)
best=ok.nsmallest(8,"chi2_planck").to_dict("records")
summary={"Q0":Q0,"center_chi2":center,"best":best[0],
         "best_delta_vs_center":float(ok.chi2_planck.min()-center),
...
(OUT/"snclosure223_q_reclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("SNC223Q_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNC223Q_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
