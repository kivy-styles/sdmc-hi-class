...
OUT=Path("output/ztbest_qbest_fullplik")
...
CANDIDATES=[
    dict(id="ztbest_qbest",H0=70.79608815124146,ob=0.022083219194622913,
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("ZTQBEST_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("ZTQBEST_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("ZTQBEST_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("ZTQBEST_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("ZTQBEST_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"ztbest_qbest_fullplik.csv",index=False)
...
(OUT/"ztbest_qbest_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTQBEST_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
