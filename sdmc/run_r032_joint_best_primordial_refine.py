...
OUT=Path("output/joint_best_primordial"); OUT.mkdir(parents=True,exist_ok=True)
...
AF=0.0525
ZC=4.6
WIDTH=0.680590909090909

NS_VALUES=[0.960,0.962,0.964,0.966]
TAU_VALUES=[0.0544,0.0564,0.0584,0.0604,0.0610,0.0624]
LNAS_VALUES=[3.066,3.070,3.074,3.078]
...
design=[
 ("exact_best",0.964,3.074,0.0610),
 ("tau0584_sameAs",0.964,3.074,0.0584),
 ("tau0564_sameAs",0.964,3.074,0.0564),
 ("tau0584_qshift",0.964,3.0688,0.0584),
 ("ns962_tau0584",0.962,3.074,0.0584),
]
for ns in NS_VALUES:
...
df=pd.DataFrame(rows); df.to_csv(OUT/"joint_best_primordial_profile.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
if ok.empty: raise SystemExit("No stable physical+primordial candidates")
keep=set(["exact_best","tau0584_sameAs","tau0564_sameAs","tau0584_qshift","ns962_tau0584"])
...
short.to_csv(OUT/"joint_best_primordial_shortlist.csv",index=False)
