...
OUT=Path("output/partial_zeq_boundbest_desi")
...
MODELS={
    ...
    "PARTIAL_ZEQ_BOUNDBEST":dict(
        kind="sdmc",
        H0=70.77503249553342,
        ob=0.02207015515212146,
        oc=0.12318755212377465,
        As=2.132477632355162e-9,
        ns=0.9625227132590487,
        tau=0.055202901571989066,
        AF=0.051437500000000004,
        zc=2.9078125,
        width=0.503125,
        dfloor=0.04378125,
        lam=18.05625,
        zt=17.045312499999998,
    ),
}
...
print("PZEQBOUNDBEST_DESI_Z",zvals,flush=True)
...
def ini_sdmc(root,m):
    ...
    parameters_smg = {m['AF']:.17g},{m['zc']:.17g},{m['width']:.17g},0.34231919445927034,1.0,{m['dfloor']:.17g}
    expansion_model = sdmc_full
    expansion_smg = {OX:.17g},{m['lam']:.17g},{m['zt']:.17g},0.5,0.01105624999,0.25,0.01951933685,1.5
...
for label,m in MODELS.items():
    ...
    print("PZEQBOUNDBEST_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("PZEQBOUNDBEST_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("PZEQBOUNDBEST_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("PZEQBOUNDBEST_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("PZEQBOUNDBEST_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "PARTIAL_ZEQ_BOUNDBEST":totals["PARTIAL_ZEQ_BOUNDBEST"],
    "delta_partial_zeq_boundbest_minus_lcdm":totals["PARTIAL_ZEQ_BOUNDBEST"]-totals["LCDM"],
}
pd.DataFrame(allrows).to_csv(OUT/"partial_zeq_boundbest_desi_profile.csv",index=False)
(OUT/"partial_zeq_boundbest_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQBOUNDBEST_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
