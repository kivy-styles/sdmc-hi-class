#!/usr/bin/env python3
"""
Targeted amplitude/reionization refinement on the physical No-Slip
Planck-mass ridge while retaining the original Part-VIII ordinary geometry.

This tests the missing compensation suggested by the component decomposition:
keep the physical ridge's strong high-l fit, then vary A_s and tau to remove
the large A_planck calibration cost without moving to the acoustic endpoint.

Fixed structural/background coordinates:
  lambda_e=17.925, z_t=17.775, Dfloor=0.045
Fixed physical No-Slip ridge:
  A_F=0.0661676420550793, z_c=3.0664408076554537,
  width=0.5638187950477004
Fixed original geometry:
  H0=70.8514, omega_b=0.02239952,
  omega_cdm=0.12444227328918850, n_s=0.964

Scan only tau and ln(1e10 A_s). No artificial A_L and no slip departure.
"""
...
OUT=Path("output/physical_oldgeom_amp"); OUT.mkdir(parents=True,exist_ok=True)
...
TAUS=[0.0504,0.0524,0.0544,0.0564,0.0584,0.0604]
LNAS=[3.048,3.052,3.056,3.060,3.064,3.068,3.072]
...
print("POA_LCDM",base,flush=True)
...
for tau in TAUS:
  for lnAs in LNAS:
    tag=f"t{tau:.5f}_a{lnAs:.6f}".replace(".","p")
    root=str(OUT/(tag+"_")); ip=OUT/(tag+".ini"); ip.write_text(ini(root,lnAs,tau))
    cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=240)
    rec=dict(id=tag,tau_reio=tau,ln10As=lnAs,A_s=math.exp(lnAs)/1e10,
    ...
    rows.append(rec); print("POA_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"physical_oldgeom_amplitude.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
if ok.empty: raise SystemExit("no stable points")
print("POA_BEST_TOTAL",ok.nsmallest(15,"chi2_total").to_dict("records"),flush=True)
print("POA_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("POA_BEST_CAL",ok.nsmallest(15,"chi2_cal").to_dict("records"),flush=True)
