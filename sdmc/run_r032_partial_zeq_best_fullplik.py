...
OUT=Path("output/partial_zeq_ztbest_fullplik")
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("ZTBESTQFINE_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("ZTBESTQFINE_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("ZTBESTQFINE_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("ZTBESTQFINE_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("ZTBESTQFINE_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"ztbest_qfine_fullplik.csv",index=False)
...
(OUT/"ztbest_qfine_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTBESTQFINE_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
