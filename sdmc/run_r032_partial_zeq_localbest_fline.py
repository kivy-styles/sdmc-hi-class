#!/usr/bin/env python3
"""
Partial-equality acoustic-locked edge refinement around the exact sobol016 winner.
...
OUT=Path("output/partial_zeq_localbest_fline"); OUT.mkdir(parents=True,exist_ok=True)
...
print("PZEQFLINE_TARGET",json.dumps({**der0,**stab0,**base},sort_keys=True),flush=True)
...
def candidate(tag,f,af,zc,w,df,lam,zt):
    ...
    if len(vals)<2:
        ...
        print("PZEQFLINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
        return rec
...
rows=[]
# Final enclosure line through the hard-likelihood local-best structure.
FS=[0.06,0.08,0.10,0.12,0.13,0.14,0.15,0.16,0.18,0.20,0.22,0.25]
for i,f in enumerate(FS):
    rows.append(candidate(f"fline{i:02d}",float(f),
                          0.052875,2.878125,0.54625,0.03825,
                          18.278125,16.962500000000002))

df=pd.DataFrame(rows)
df.to_csv(OUT/"partial_zeq_localbest_fline.csv",index=False)
ok=df[(df.status=="OK") & (df.stable_subluminal==True)].copy()
best=ok.nsmallest(12,"chi2_planck").to_dict("records")
print("PZEQFLINE_POINTS",json.dumps(ok.sort_values("f").to_dict("records"),sort_keys=True),flush=True)
summary={"target_ellA":TARGET,
         "best":best[0] if best else None,
         "best_delta_vs_geom":float(ok.delta_planck_vs_geom.min()) if len(ok) else None,
         "best_f":float(ok.loc[ok.chi2_planck.idxmin(),"f"]) if len(ok) else None}
(OUT/"partial_zeq_localbest_fline_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQFLINE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
