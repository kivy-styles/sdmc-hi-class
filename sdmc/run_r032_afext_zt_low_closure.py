#!/usr/bin/env python3
"""
Lower-z_t closure around the AFext zt0160 structural winner.

Motivation:
  The AF-extension screen improved on localref008 at z_t=16.10, close to
  the lower z_t edge of the sampled range. Hold the structural transition
  at the stable AFext winner and continue z_t downward to determine whether
  this direction is genuinely enclosed before any Kp work.
...
OUT=Path("output/r032_afext_zt_low_closure"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5
TARGET=301.6798271349195

H00=70.79091039066579
...
LOW=np.array([0.0280,3.35,0.360,0.0430,16.00],float)
HIGH=np.array([0.0385,4.05,0.435,0.0620,16.38],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("ZTLOW_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("ZTLOW_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec

rows=[]
base=candidate("zt0160",AF0,ZC0,W0,DF0,ZT0)
if base.get("status")!="OK": raise RuntimeError("zt0160 baseline failed")
BASE=float(base["chi2_planck"]); rows.append(base)

ZTS=[15.40,15.55,15.70,15.80,15.90,15.95,16.00,16.025,16.05,16.075,16.10,16.125,16.15,16.20]
for i,zt in enumerate(ZTS):
    if abs(zt-ZT0)<1e-12: continue
    rows.append(candidate(f"zt_{i:02d}",AF0,ZC0,W0,DF0,float(zt)))

for r in rows:
    if r.get("status")=="OK":
        r["delta_planck_vs_zt0160"]=r["chi2_planck"]-BASE
        r["delta_high_vs_zt0160"]=r["chi2_high"]-base["chi2_high"]
        r["delta_lowT_vs_zt0160"]=r["chi2_lowT"]-base["chi2_lowT"]
        r["delta_lowE_vs_zt0160"]=r["chi2_lowE"]-base["chi2_lowE"]
        r["delta_lensing_vs_zt0160"]=r["chi2_lensing"]-base["chi2_lensing"]
        r["delta_cal_vs_zt0160"]=r["chi2_cal"]-base["chi2_cal"]

df=pd.DataFrame(rows)
df.to_csv(OUT/"r032_afext_zt_low_closure.csv",index=False)
...
summary={
 ...
 "best_delta_vs_zt0160":float(ok.chi2_planck.min()-BASE) if len(ok) else None,
 "best_at_lower_edge":bool(len(ok) and abs(float(ok.nsmallest(1,"chi2_planck").iloc[0].z_t)-min(ZTS))<1e-9)
}
(OUT/"r032_afext_zt_low_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTLOW_BEST",json.dumps(best,sort_keys=True),flush=True)
print("ZTLOW_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
