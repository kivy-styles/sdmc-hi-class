#!/usr/bin/env python3
"""
Final primordial closure around the exact Part-IX partial-equality z_t-best.
...
OUT=Path("output/ztbest_primordial_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
# n_s, tau, Q
LOW=np.array([0.9560,0.0450,Q0-0.018],float)
HIGH=np.array([0.9700,0.0660,Q0+0.018],float)
...
def run(tag,ns,tau,Q):
    ...
    print("ZTPRIM_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec

rows=[]
base=run("ztbest_center",NS0,TAU0,Q0)
...
for rec in rows:
    if rec.get("status")=="OK":
        rec["delta_planck_vs_ztbest"]=rec["chi2_planck"]-BASE
        rec["delta_high_vs_ztbest"]=rec["chi2_high"]-base["chi2_high"]
        rec["delta_lowT_vs_ztbest"]=rec["chi2_lowT"]-base["chi2_lowT"]
        rec["delta_lowE_vs_ztbest"]=rec["chi2_lowE"]-base["chi2_lowE"]
        rec["delta_lensing_vs_ztbest"]=rec["chi2_lensing"]-base["chi2_lensing"]
        rec["delta_cal_vs_ztbest"]=rec["chi2_cal"]-base["chi2_cal"]

df=pd.DataFrame(rows)
df.to_csv(OUT/"ztbest_primordial_closure.csv",index=False)
...
summary={
  ...
  "best_delta_vs_ztbest":float(ok.chi2_planck.min()-BASE) if len(ok) else None
}
(OUT/"ztbest_primordial_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTPRIM_BEST_PLANCK",json.dumps(best,sort_keys=True),flush=True)
print("ZTPRIM_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
