...
OUT=Path("output/kp_core_local_enclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
KPSEED=dict(
 H0=69.50935515603771,ob=0.02205,oc=0.123,ns=0.967,tau=0.0585,
 Q=2.935,
 af=0.0235,zc=5.4,width=0.395,D0=0.2,pD=0.925,df=0.06625,
 lam=18.65,zt=19.9,dNt=0.76,A=0.01275,tauA=0.63,
 B=0.007625,tauB=1.9875
)
...
LOW=np.array([
 0.02190,0.1220,0.9630,0.0540,2.928,
 0.0180,4.80,0.30,0.14,0.75,0.052,
 18.0,19.0,0.64,0.009,0.46,0.0050,1.50
],float)
HIGH=np.array([
 0.02220,0.1242,0.9705,0.0625,2.944,
 0.0300,5.85,0.50,0.29,1.16,0.090,
 19.35,20.8,0.88,0.0175,0.78,0.0115,2.35
],float)
...
print("KPLOCAL_REFERENCE",json.dumps({**ref,**REF},sort_keys=True),flush=True)
...
def acoustic_candidate(tag,c0):
    ...
    if pair is None:
        ...
        print("KPLOCAL_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    (h1,g1),(h2,g2)=pair
    ...
    print("KPLOCAL_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec

rows=[]
rows.append(acoustic_candidate("kp_sobol003_center",KPSEED))

sam=qmc.Sobol(d=len(NAMES),scramble=False)
pts=qmc.scale(sam.random_base2(m=6),LOW,HIGH)  # 64 local Kp-like points
for i,p in enumerate(pts):
    c={k:float(v) for k,v in zip(NAMES,p)}
    rows.append(acoustic_candidate(f"sobol{i:03d}",c))

df=pd.DataFrame(rows); df.to_csv(OUT/"kp_core_local_enclosure.csv",index=False)
...
(OUT/"kp_core_local_enclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("KPLOCAL_BEST",json.dumps(best,sort_keys=True),flush=True)
print("KPLOCAL_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
