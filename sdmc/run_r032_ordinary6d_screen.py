...
# Profile the ordinary background around the exact leader while keeping
# the newly identified primordial optimum n_s=0.964, tau=0.062 and fixed A_s.
# The local acoustic constraint is
#   d ell_A ~= -0.836 dH0 + 784 d omega_b - 293 d omega_c,
# so omega_c is adjusted to stay close to the same acoustic scale.
HGRID=[70.8514,70.70,70.55,70.40,70.25]
DOB=[-0.00020,-0.00010,0.0,0.00010]
design=[]
for H0 in HGRID:
    dH=H0-CURRENT[0]
    for dob in DOB:
        ...
        x[4]=3.076
        x[5]=0.062
        tag=f"H{H0:.2f}_dob{dob:+.5f}".replace(".","p").replace("+","p").replace("-","m")
        design.append((tag,x))
...
meta={
    ...
    "n_design":len(design),
    "note":"tau062 acoustic-manifold profile; final points require exact covariant replay + full Plik + raw DESI FS",
}
