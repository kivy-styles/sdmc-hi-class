...
CANDS = {
 "s117_g022": dict(dir="s117_g022", omega_b=0.022083219194622913, lambda_e=18.40625,
              pd_fair=-0.10979459350505749),
}
...
for label,meta in CANDS.items():
    ...
    row=dict(candidate=label,H0=H0,rd_Mpc=rd,
             ...
             sn_pp_delta_vs_local021=sn["pantheonplus"]-local_sn["pantheonplus"],
             sn_u3_delta_vs_local021=sn["union3"]-local_sn["union3"],
             sn_desy5_delta_vs_local021=sn["desy5"]-local_sn["desy5"],
             ...
             bao_chi2=bchi,bao_delta_vs_local021=bchi-local_bao_chi,
...
print("UNIFORM_BATTERY_LOCAL021",json.dumps(local,sort_keys=True),flush=True)
...
for r in rows:
    verdict.append(dict(
      ...
      Planck_DESI_crosses_local021=(r["pd_fair"]<0),
      Pantheon_joint_crosses_local021=(r["joint_pd_pp_fair"]<0),
      Union3_joint_crosses_local021=(r["joint_pd_u3_fair"]<0),
      DESY5_joint_crosses_local021=(r["joint_pd_desy5_fair"]<0),
