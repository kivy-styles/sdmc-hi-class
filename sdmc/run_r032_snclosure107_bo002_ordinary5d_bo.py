...
LOW=np.array([OB0-0.00045,OC0-0.0030,NS0-0.0090,Q0-0.0140,max(0.038,TAU0-0.0120)])
HIGH=np.array([OB0+0.00045,OC0+0.0030,NS0+0.0090,Q0+0.0140,min(0.078,TAU0+0.0120)])
...
def finish(rec):
    if rec["status"]!="OK":
        rec["delta_planck_lite"]=999.; rec["pd_fair_proxy"]=999.
        rec["joint_pp_proxy"]=999.; rec["joint_u3_proxy"]=999.; rec["joint_d5_proxy"]=999.
        rec["second_joint_proxy"]=999.; rec["goal_score"]=999.
        return rec
    dp=rec["chi2_planck"]-CPL
    rec["delta_planck_lite"]=float(dp)
    rec["pd_fair_proxy"]=float(BO002_PD_FAIR+dp)
    rec["joint_pp_proxy"]=rec["pd_fair_proxy"]+rec["sn_delta_pantheonplus"]
    rec["joint_u3_proxy"]=rec["pd_fair_proxy"]+rec["sn_delta_union3"]
    rec["joint_d5_proxy"]=rec["pd_fair_proxy"]+rec["sn_delta_desy5"]
    vals=sorted([rec["joint_pp_proxy"],rec["joint_u3_proxy"],rec["joint_d5_proxy"]])
    rec["second_joint_proxy"]=float(vals[1])
    rec["goal_score"]=float(vals[1])
    return rec

center=finish(center)
rows=[center]

# Initial 48 Sobol points + a few axis checks.
sob=qmc.Sobol(d=5,scramble=True,seed=1070025)
U=sob.random_base2(m=6)[:48]
...
for it in range(18):
    ...
    gp=GaussianProcessRegressor(kernel=kernel,alpha=1e-8,normalize_y=True,n_restarts_optimizer=2,random_state=10700+it)
    gp.fit(Xs,ya)
    rng=np.random.default_rng(107200+it)
