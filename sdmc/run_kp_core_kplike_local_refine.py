...
OUT=Path("output/kp_core_kplike_local"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([
  69.6, 0.02195, 0.1180, 0.957, 0.047, 3.035,
  0.030, 2.3, 0.50, 0.18, 0.45, 0.032,
  17.4, 15.8, 0.38, 0.018, 0.0045, 1.00
],float)
HIGH=np.array([
  71.9, 0.02260, 0.1238, 0.972, 0.065, 3.078,
  0.072, 4.8, 1.05, 0.46, 1.35, 0.068,
  21.0, 20.8, 0.72, 0.042, 0.0115, 1.90
],float)
...
def run(tag,c):
    ...
    print("KPLIKE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
print("KPLIKE_CURRENT_LEADER",json.dumps(leader,sort_keys=True),flush=True)
...
# Kp compressed-CMB lobe centers, but with the current stable independent
# kinetic/No-Slip structure. These isolate the historical A/B basin from the
# known unstable frozen perturbation completion.
for tag,A0,B0 in [
    ("kplike_refined_AB",0.0280,0.0070),
    ("kplike_joint_AB",0.0350,0.0075),
    ("kplike_cmbfloor_AB",0.0400,0.0075),
]:
    cc=dict(LEADER)
    cc["A"]=A0; cc["B"]=B0
    rows.append(run(tag,cc))
...
df.to_csv(OUT/"kp_core_kplike_local_refine.csv",index=False)
ok=df[(df.status=="OK") & (df.stable_subluminal==True)].copy()
best=ok.nsmallest(12,"chi2_planck").to_dict("records")
print("KPLIKE_BEST_PLANCK",json.dumps(best,sort_keys=True),flush=True)
summary={
...
(OUT/"kp_core_kplike_local_summary.json").write_text(json.dumps(summary,indent=2))
print("KPLIKE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
