...
(OUT/"lcdm6d_design.json").write_text(json.dumps({
    "names":NAMES,"current":CURRENT.tolist(),"reference":REFERENCE.tolist(),
    "low":LOW.tolist(),"high":HIGH.tolist(),"n_design":len(design)
},indent=2))
...
for ic,(tag,x) in enumerate(design,1):
    ...
    print("LCDM6D_POINT",json.dumps(rec,sort_keys=True),flush=True)
    rows.append(rec)
...
keep=set(["current_coords","reference","line25","line50","line75"])
...
short.to_csv(OUT/"lcdm6d_shortlist.csv",index=False)
print("LCDM6D_BEST_PLANCK",ok.nsmallest(10,"chi2_planck")[NAMES+["id","chi2_planck","A_planck","sigma8"]].to_dict("records"),flush=True)
print("LCDM6D_BEST_PP",ok.nsmallest(10,"planck_pp")[NAMES+["id","planck_pp","chi2_planck","chi2_pantheonplus"]].to_dict("records"),flush=True)
print("LCDM6D_BEST_UNION3",ok.nsmallest(10,"planck_union3")[NAMES+["id","planck_union3","chi2_planck","chi2_union3"]].to_dict("records"),flush=True)
print("LCDM6D_BEST_DESY5",ok.nsmallest(10,"planck_desy5")[NAMES+["id","planck_desy5","chi2_planck","chi2_desy5"]].to_dict("records"),flush=True)
