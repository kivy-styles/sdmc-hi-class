...
OUT=Path("output/partial_zeq_local010_desi")
...
print("PZEQLOCAL010_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("PZEQLOCAL010_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("PZEQLOCAL010_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("PZEQLOCAL010_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("PZEQLOCAL010_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("PZEQLOCAL010_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "PARTIAL_ZEQ_LOCAL010":totals["PARTIAL_ZEQ_LOCAL010"],
    "delta_partial_zeq_local010_minus_lcdm":totals["PARTIAL_ZEQ_LOCAL010"]-totals["LCDM"],
}
pd.DataFrame(allrows).to_csv(OUT/"partial_zeq_local010_desi_profile.csv",index=False)
(OUT/"partial_zeq_local010_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQLOCAL010_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
