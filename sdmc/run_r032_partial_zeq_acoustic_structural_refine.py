...
OUT=Path("output/partial_zeq_acoustic_structural"); OUT.mkdir(parents=True,exist_ok=True)
...
print("PZEQ_TARGET",json.dumps({**der0,**stab0,**base},sort_keys=True),flush=True)
...
rows=[]
rows.append(candidate("center_f015",0.15,AF0,ZC0,W0,DF0,LAM0,ZT0))
...
df=pd.DataFrame(rows); df.to_csv(OUT/"partial_zeq_acoustic_structural.csv",index=False)
ok=df[(df.status=="OK") & (df.stable_subluminal==True)].copy()
best=ok.nsmallest(10,"chi2_planck").to_dict("records")
print("PZEQ_BEST_PLANCK",json.dumps(best,sort_keys=True),flush=True)
summary={"target_ellA":TARGET,"center":rows[0],
         "best":best[0] if best else None,
         "best_delta_vs_geom":float(ok.delta_planck_vs_geom.min()) if len(ok) else None}
(OUT/"partial_zeq_acoustic_structural_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQ_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
