#!/usr/bin/env python3
"""
Correlated stability-ridge closure around the final r032 zt-best + Qfine-best point.

Motivation:
  A one-dimensional z_c continuation found a small Planck-screen improvement
  near z_c=2.96, but the best point sat close to the c_s^2 stability floor and
  lower z_c became unstable. This test asks whether coordinated changes in
  A_F, z_c, width, D_floor and z_t can move along that stability ridge and
  improve the CMB fit while preserving the same acoustic scale.
...
OUT=Path("output/r032_stability_ridge_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.0470,2.88,0.475,0.0385,16.32],float)
HIGH=np.array([0.0530,3.02,0.555,0.0480,16.66],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("RIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("RIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
# Explicit points around the observed z_c stability edge and A_F direction.
anchors=[
 ("zc096",AF0,2.96,W0,DF0,ZT0),
 ("af_low1",0.0490,ZC0,W0,DF0,ZT0),
 ("af_low2",0.04825,ZC0,W0,DF0,ZT0),
 ("afzc1",0.0490,2.96,0.525,0.0440,16.48),
 ("afzc2",0.0485,2.94,0.535,0.0455,16.46),
 ("afzc3",0.0480,2.92,0.545,0.0470,16.44),
]
for a in anchors:
    rows.append(candidate(*a))

sam=qmc.Sobol(d=5,scramble=True,seed=20260922)
...
df.to_csv(OUT/"r032_stability_ridge_closure.csv",index=False)
...
(OUT/"r032_stability_ridge_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("RIDGE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("RIDGE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
