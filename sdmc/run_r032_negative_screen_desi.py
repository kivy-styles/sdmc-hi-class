...
OUT=Path("output/negative_screen_desi")
...
MODELS={
    ...
    "SOBOL049":dict(
        ...
        As=2.114656135507997e-09,
        ns=0.9614044621046632,
        tau=0.05464880801877007,
        AF=0.05709640212077647,
        zc=2.8532181778922676,
        width=0.575869657304138,
    ),
    "SOBOL009":dict(
        kind="sdmc",
        H0=70.79871074482799,
        ob=0.02221026752647944,
        oc=0.12381796840205787,
        As=2.1321093695623593e-09,
        ns=0.9633178803175687,
        tau=0.0596811323207803,
        AF=0.05435267057083547,
        zc=3.1564399330876767,
        width=0.517175733782351,
    ),
}
...
print("NS_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("NS_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("NS_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("NS_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("NS_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("NS_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
pd.DataFrame(allrows).to_csv(OUT/"negative_screen_desi_profile.csv",index=False)
(OUT/"negative_screen_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("NS_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
