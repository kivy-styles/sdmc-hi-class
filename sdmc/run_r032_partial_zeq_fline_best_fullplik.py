...
OUT=Path("output/partial_zeq_fline_best_fullplik")
...
CANDIDATES=[
    dict(id="partial_zeq_fline_best",H0=70.92621543826743,ob=0.022098748150803885,
         oc=0.12276692102005238,ns=0.9625227132590487,tau=0.055202901571989066,
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("PZEQFLINEBEST_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("PZEQFLINEBEST_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("PZEQFLINEBEST_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("PZEQFLINEBEST_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("PZEQFLINEBEST_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"partial_zeq_fline_best_fullplik.csv",index=False)
...
(OUT/"partial_zeq_fline_best_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQFLINEBEST_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
