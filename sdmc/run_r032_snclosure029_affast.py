...
OUT=Path("output/snclosure029_affast"); OUT.mkdir(parents=True,exist_ok=True)
...
""")
rows=[]
for i,AF in enumerate(np.linspace(0.024,0.040,17)):
 ...
 rows.append(r); print("SN029AF_POINT",json.dumps(r,sort_keys=True),flush=True)
df=pd.DataFrame(rows); center=float(df.iloc[np.argmin(abs(df.A_F-AF0))].chi2_planck)
df["delta_vs_nearest_center"]=df.chi2_planck-center; df.to_csv(OUT/"snclosure029_affast.csv",index=False)
ok=df[df.status=="OK"].sort_values("chi2_planck"); best=ok.head(8).to_dict("records")
print("SN029AF_BEST",json.dumps(best,sort_keys=True),flush=True)
