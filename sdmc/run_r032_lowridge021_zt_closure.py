#!/usr/bin/env python3
"""
One-dimensional z_t closure around the exact-validated lowridge021 winner.

Motivation:
  lowridge021 reduced the exact P+D residual to about 1.09 chi2 and landed at
  z_t=16.314355, essentially on the lower edge of the search box. This scan
  extends z_t downward at the validated lowridge021 structural point before
  launching another correlated refinement.
...
OUT=Path("output/r032_lowridge021_zt_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("ZTCL_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("ZTCL_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
ZTS=[15.70,15.80,15.90,16.00,16.08,16.12,16.16,16.20,16.24,16.27,16.29,16.30,
     16.314355247840286,16.33,16.35,16.38,16.42,16.46,16.50]
for i,zt in enumerate(ZTS):
    if abs(zt-ZT0)<1e-10: continue
...
df.to_csv(OUT/"r032_lowridge021_zt_closure.csv",index=False)
...
(OUT/"r032_lowridge021_zt_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTCL_BEST",json.dumps(best,sort_keys=True),flush=True)
print("ZTCL_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
