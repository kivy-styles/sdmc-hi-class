...
OUT=Path("output/partial_zeq_edge_best_desi")
...
MODELS={
    ...
    "PARTIAL_ZEQ_EDGE_BEST":dict(
        kind="sdmc",
        H0=70.86703259055196,
        ob=0.022095790254388462,
        oc=0.1228104345825064,
...
print("PZEQEDGEBEST_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("PZEQEDGEBEST_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("PZEQEDGEBEST_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("PZEQEDGEBEST_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("PZEQEDGEBEST_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("PZEQEDGEBEST_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "PARTIAL_ZEQ_EDGE_BEST":totals["PARTIAL_ZEQ_EDGE_BEST"],
    "delta_partial_zeq_edge_best_minus_lcdm":totals["PARTIAL_ZEQ_EDGE_BEST"]-totals["LCDM"],
}
pd.DataFrame(allrows).to_csv(OUT/"partial_zeq_edge_best_desi_profile.csv",index=False)
(OUT/"partial_zeq_edge_best_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQEDGEBEST_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
