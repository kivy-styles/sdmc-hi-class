#!/usr/bin/env python3
"""
Equality-scale constrained, acoustic-scale-locked structural re-optimization.
...
OUT=Path("output/zeq_acoustic_locked_structural"); OUT.mkdir(parents=True,exist_ok=True)
...
for i,p in enumerate(pts):
    ...
    print("ZEQLOCK_POINT",json.dumps(rec,sort_keys=True),flush=True)
...
df.to_csv(OUT/"zeq_acoustic_locked_structural_reopt.csv",index=False)
...
for col,label in [
    ...
    print("ZEQLOCK_BEST_"+label,ok.nsmallest(10,col).to_dict("records"),flush=True)
...
(OUT/"zeq_acoustic_locked_structural_summary.json").write_text(json.dumps(summary,indent=2))
print("ZEQLOCK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
