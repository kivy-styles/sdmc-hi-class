...
# Correlated low-AF / narrow-width family.
# The current peak derivative scale is AF/width ~ 0.05014.
# For each lower AF we sample around the width that preserves this ratio,
# while also moving z_c to test stability timing.
AFS=[0.035,0.040,0.045,0.050,0.055,0.060,0.065,0.070]
ZCS=[3.8,4.4,5.0,5.6,6.2]
design=[("current",0.0715,5.0,1.426)]
for af in AFS:
    wc=af/(0.0715/1.426)
    for fac in [0.80,1.00,1.20]:
        w=wc*fac
        for zc in ZCS:
            design.append((f"af{af:.4f}_zc{zc:.2f}_w{w:.3f}",af,zc,w))
