#!/usr/bin/env python3
"""
Edge continuation of the low-width stability ridge around lowridge021.

Motivation:
  The lower-width closure improved Planck-lite by 1.74044 and its winner
  landed close to both the lower A_F and lower z_t boundaries. This scan
  continues that same stable basin to lower A_F and z_t while allowing
  z_c, width and D_floor to compensate.
...
OUT=Path("output/r032_lowridge_edge_closure"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5
TARGET=301.6798271349195

H00=70.7937310688954
...
AF0=0.0411132003464736
ZC0=3.431916184257716
W0=0.4104880520515144
D0=0.34231919445927034
DF0=0.047899333223700526
LAM=18.40625
ZT0=16.314355247840286

# A_F, z_c, width, D_floor, z_t
LOW=np.array([0.0300,3.00,0.340,0.0430,15.40],float)
HIGH=np.array([0.0445,4.30,0.480,0.0680,16.34],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("EDGE_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("EDGE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
# Explicit continuation anchors from lowridge021 toward lower A_F and z_t.
anchors=[
 ("lowridge021",0.0411132003464736,3.431916184257716,0.4104880520515144,0.047899333223700526,16.314355247840286),
 ("edge1",0.0400,3.45,0.410,0.0490,16.20),
 ("edge2",0.0385,3.50,0.405,0.0510,16.05),
 ("edge3",0.0370,3.60,0.400,0.0530,15.90),
 ("edge4",0.0350,3.70,0.395,0.0560,15.70),
 ("edge5",0.0330,3.85,0.390,0.0600,15.50),
]
for a in anchors:
...
df.to_csv(OUT/"r032_lowridge_edge_closure.csv",index=False)
...
(OUT/"r032_lowridge_edge_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("EDGE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("EDGE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
