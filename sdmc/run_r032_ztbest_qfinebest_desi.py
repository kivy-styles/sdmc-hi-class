...
OUT=Path("output/ztbest_qfinebest_desi")
...
print("ZTQFINE_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("ZTQFINE_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("ZTQFINE_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print("ZTQFINE_DESI_ROBUST_TRIAL",label,b["namespace"],iseed,float(opti.fun),
        ...
        rows.append(row)
        print("ZTQFINE_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("ZTQFINE_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "ZTBEST_QFINEBEST":totals["ZTBEST_QFINEBEST"],
    "delta_ztbest_qfinebest_minus_lcdm":totals["ZTBEST_QFINEBEST"]-totals["LCDM"],
}
pd.DataFrame(allrows).to_csv(OUT/"ztbest_qfinebest_desi_profile.csv",index=False)
(OUT/"ztbest_qfinebest_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTQFINE_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
