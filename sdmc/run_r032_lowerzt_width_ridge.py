#!/usr/bin/env python3
"""
Lower-z_t / narrow-width continuation of the final r032 Qfine stability ridge.
...
OUT=Path("output/r032_lowerzt_width_ridge"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.15,0.0480,2.88,0.430,0.0440,15.75],float)
HIGH=np.array([0.30,0.0525,3.04,0.500,0.0570,16.38],float)
...
def candidate(tag,f,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LZRIDGE_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
...
anchors=[
 ("ridgebest",F0,0.0489503458160907,2.9896906245313586,0.48081518225371833,0.0469228272177279,16.332678021918984),
 ("lowerzt1",F0,0.0490,2.99,0.472,0.0480,16.20),
 ("lowerzt2",0.22,0.0490,2.98,0.465,0.0500,16.05),
 ("lowerzt3",0.24,0.0495,2.96,0.455,0.0520,15.90),
 ("narrow1",0.20,0.0500,2.94,0.445,0.0540,16.10),
]
for a in anchors: rows.append(candidate(*a))

sam=qmc.Sobol(d=6,scramble=True,seed=20260923)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_lowerzt_width_ridge.csv",index=False)
...
(OUT/"r032_lowerzt_width_ridge_summary.json").write_text(json.dumps(summary,indent=2))
print("LZRIDGE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LZRIDGE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
