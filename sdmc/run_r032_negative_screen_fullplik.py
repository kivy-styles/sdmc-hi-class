...
OUT=Path("output/negative_screen_fullplik")
...
CANDIDATES=[
    dict(id="sobol049",H0=70.6661848725751,ob=0.02213832986768335,
         oc=0.12419066331610083,ns=0.9614044621046632,tau=0.05464880801877007,
         lnAs=3.051477308589965,AF=0.05709640212077647,zc=2.8532181778922676,width=0.575869657304138),
    dict(id="sobol009",H0=70.79871074482799,ob=0.02221026752647944,
         oc=0.12381796840205787,ns=0.9633178803175687,tau=0.0596811323207803,
         lnAs=3.0596968970261513,AF=0.05435267057083547,zc=3.1564399330876767,width=0.517175733782351),
]
def table(path):
    import re
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("NSFP_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("NSFP_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("NSFP_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("NSFP_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("NSFP_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"negative_screen_fullplik.csv",index=False)
...
(OUT/"negative_screen_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("NSFP_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
