#!/usr/bin/env python3
"""
Targeted amplitude/reionization refinement on the best physical No-Slip
Planck-mass ridge, using the best acoustic-degeneracy ordinary cosmology.

Fixed structural/background coordinates:
  lambda_e=17.925, z_t=17.775, Dfloor=0.045
Fixed physical No-Slip ridge:
  A_F=0.0573794336710125, z_c=3.0902165911160413,
  width=0.5979282500222326
Fixed acoustic geometry:
...
OUT=Path("output/acoustic_physical_amp"); OUT.mkdir(parents=True,exist_ok=True)
...
TAUS=[0.0524,0.0544,0.0564,0.0584,0.0614,0.06456347454525531]
LNAS=[3.052,3.0570,3.060,3.064,3.068,3.072,3.0773358772153037,3.082]
...
print("APA_LCDM",base,flush=True)
...
for tau in TAUS:
  for lnAs in LNAS:
    ...
    rows.append(rec); print("APA_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"acoustic_physical_amplitude_refine.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
if ok.empty: raise SystemExit("no stable points")
print("APA_BEST_TOTAL",ok.nsmallest(15,"chi2_total").to_dict("records"),flush=True)
print("APA_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("APA_BEST_CAL",ok.nsmallest(15,"chi2_cal").to_dict("records"),flush=True)
