...
OUT=Path("output/r032_earlyzt_ridge_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("EARLYZT_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("EARLYZT_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"r032_earlyzt_ridge_closure.csv",index=False)
...
(OUT/"r032_earlyzt_ridge_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("EARLYZT_BEST",json.dumps(best,sort_keys=True),flush=True)
print("EARLYZT_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
