...
x0=np.array([
 1.0,1.0002,0.99805,67.,0.05,7.,3.,
 8.6,10.6,23.5,91.9,257.,47.,40.,104.,
 .130,.130,.46,.207,.69,1.938
],float)
# Exact-leader full-Plik nuisance optimum from run 35510661121.
leader_seed=np.array([
...
# Generate all spectra first.
models=[]
for tag,tau in [
 ("current",0.0544),("t0600",0.0600),("t0610",0.0610),
 ("t0620",0.0620),("t06225",0.06225),("t0630",0.0630),("t0640",0.0640)
]:
    root=str(OUT/(tag+"_")); ip=OUT/(tag+".ini"); ip.write_text(sdmc_ini(tau,root))
    cp=subprocess.run(["./class",str(ip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=300)
    if cp.returncode: raise RuntimeError(f"{tag} CLASS failed\n"+cp.stdout[-3000:])
    models.append((tag,tau,Path(root+"00_cl_lensed.dat")))

lroot=str(OUT/"lcdm_"); lip=OUT/"lcdm.ini"; lip.write_text(lcdm_ini(lroot))
cp=subprocess.run(["./class",str(lip)],stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True,timeout=300)
if cp.returncode: raise RuntimeError(cp.stdout[-3000:])
lcdm_path=Path(lroot+"00_cl_lensed.dat")
...
def prior_chi2(p):
    c=sum(((p[n]-mu)/sig)**2 for n,(mu,sig) in gauss.items())
...
lcdm,lx=profile("LCDM",lcdm_path,[x0,leader_seed])
rows=[]
prev=leader_seed
for tag,tau,path in models:
    displaced=prev.copy(); displaced[0]=1.0
    row,x=profile(tag,path,[prev,leader_seed,x0,displaced])
    row["tau_reio"]=tau
