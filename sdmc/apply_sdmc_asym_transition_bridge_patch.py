...
The third mode tests whether the stability obstruction belongs to the frozen
Planck-mass trajectory rather than to the exact No-Slip relation.
"""
from pathlib import Path


PATH = "gravity_smg/gravity_models_smg.c"
...
def replace_once(old: str, new: str, label: str) -> None:
    ...
    if n == 0 and new in text:
        ...
        return
    if n != 1:
        raise RuntimeError(f"{PATH}: expected one {label} anchor, found {n}\n{old}")
    p.write_text(text.replace(old, new), encoding="utf-8")
    print(f"{PATH}: {label} patched")


# 1. Expand the Kp bridge parser from six to seven parameters.  Deliberately
# use literal newlines here; a previous helper accidentally used Python source
# line continuations inside these strings and therefore searched for one long
# line that can never occur in gravity_models_smg.c.
replace_once(
...
# 2. Give the runtime closure independent edge widths and reserve z_off<0 for
# the localized ln F diagnostic.
replace_once(
...
# 3. Replace the symmetric finite pulse by three explicit bridge modes.
replace_once(
"""    double Nc = -log(1.+zc);
...
"""    double Nc = -log(1.+zc);
    ...
    double bra, bra_N;
    if (zoff < 0.) {
      /* Localized Planck-mass bridge in ln F. q=sech^2(u) vanishes on both
       * sides, so the original Kp normalization is recovered exactly. */
      double u = (N-Non)/won;
      double tb = tanh(u);
