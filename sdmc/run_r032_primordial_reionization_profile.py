#!/usr/bin/env python3
"""
Focused final-r032 primordial/reionization profile.

Scans (n_s, ln(1e10 A_s), tau_reio) at the exact r032 structural/background
point and current H0, omega_b, omega_cdm. This directly tests the compensation
suggested by Planck diagnostics: tau up, A_s slightly down, n_s adjusted.

Planck screening likelihood:
  Plik-lite + lowT + lowE + lensing with A_planck profiled.
No artificial A_L is used.
"""
...
OUT=Path("output/primordial_reionization"); OUT.mkdir(parents=True,exist_ok=True)
...
LNAS_VALUES=[3.058,3.063,3.068,3.073,3.078]
...
def score(path):
    ...
    opt=minimize_scalar(lambda A:pcs(A)[-1],bounds=(0.97,1.03),
...
print("PRIMREIO_LCDM",base,flush=True)
...
design=[("current",0.964,3.076,0.0544)]
for ns in NS_VALUES:
...
for i,(tag,ns,lnAs,tau) in enumerate(design,1):
    ...
    rec=dict(id=tag,index=i,n_s=ns,ln10As=lnAs,A_s=math.exp(lnAs)/1e10,
             tau_reio=tau,Q=lnAs-2*tau,returncode=cp.returncode,status="FAIL")
    ...
    rows.append(rec); print("PRIMREIO_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"primordial_reionization_profile.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
if ok.empty: raise SystemExit("No stable primordial/reionization candidates")
keep=set(["current"])
...
short.to_csv(OUT/"primordial_reionization_shortlist.csv",index=False)
print("PRIMREIO_BEST_TOTAL",ok.nsmallest(20,"chi2_total").to_dict("records"),flush=True)
print("PRIMREIO_BEST_HIGH",ok.nsmallest(15,"chi2_high").to_dict("records"),flush=True)
print("PRIMREIO_BEST_LENS",ok.nsmallest(15,"chi2_lensing").to_dict("records"),flush=True)
print("PRIMREIO_SHORTLIST",short.to_dict("records"),flush=True)
