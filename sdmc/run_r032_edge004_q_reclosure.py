...
OUT=Path("output/edge004_q_reclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("E04Q_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"edge004_q_reclosure.csv",index=False)
...
(OUT/"edge004_q_reclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("E04Q_BEST",json.dumps(best,sort_keys=True),flush=True)
print("E04Q_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
