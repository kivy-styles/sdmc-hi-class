...
OUT=Path("output/ztbest_qaxis02_fullplik")
...
CANDIDATES=[
    dict(id="ztbest_qaxis02",H0=70.79608815124146,ob=0.022083219194622913,
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("ZTQ02_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("ZTQ02_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("ZTQ02_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("ZTQ02_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("ZTQ02_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"ztbest_qaxis02_fullplik.csv",index=False)
...
(OUT/"ztbest_qaxis02_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("ZTQ02_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
