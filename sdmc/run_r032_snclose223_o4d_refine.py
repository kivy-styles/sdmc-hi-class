#!/usr/bin/env python3
"""
SN-aware ordinary-parameter refinement around SN-close223.
...
OUT=Path("output/snclose223_o4d_refine"); OUT.mkdir(parents=True,exist_ok=True)
SNROOT=Path("sn_data")
...
def candidate(tag,ob,oc,ns,Q):
    ...
    if pair is None:
        ...
        print("SNO4D_POINT",json.dumps(rec,sort_keys=True),flush=True);return rec
    (h1,g1),(h2,g2)=pair; H0=float(h1-g1*(h2-h1)/(g2-g1))
    try: root,bg,der,st=run_class(tag,H0,ob,oc,ns,Q,True)
    except Exception as e:
        rec=dict(id=tag,status="FAIL",error=str(e)[-500:]); print("SNO4D_POINT",json.dumps(rec,sort_keys=True),flush=True);return rec
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snclose223_o4d_refine.csv",index=False)
...
(OUT/"snclose223_o4d_refine_summary.json").write_text(json.dumps(summary,indent=2))
print("SNO4D_BEST",json.dumps(best,sort_keys=True),flush=True)
print("SNO4D_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
