#!/usr/bin/env python3
"""
Joint matter/equality + stability-ridge closure for the final r032 Qfine state.
...
OUT=Path("output/r032_f_ridge_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.14,0.0470,2.88,0.475,0.0385,16.32],float)
HIGH=np.array([0.30,0.0535,3.03,0.558,0.0485,16.68],float)
...
anchors=[
 ("f022",0.22,AF0,ZC0,W0,DF0,ZT0),
 ("f025",0.25,AF0,ZC0,W0,DF0,ZT0),
 ("joint1",0.22,0.0490,2.96,0.525,0.0440,16.48),
 ("joint2",0.25,0.0490,2.95,0.535,0.0450,16.46),
 ("joint3",0.28,0.0495,2.96,0.540,0.0460,16.44),
]
for a in anchors: rows.append(candidate(*a))

sam=qmc.Sobol(d=6,scramble=True,seed=20260922)
...
df=pd.DataFrame(rows); df.to_csv(OUT/"r032_f_ridge_closure.csv",index=False)
...
(OUT/"r032_f_ridge_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("FRIDGE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("FRIDGE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
