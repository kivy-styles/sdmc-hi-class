...
OUT=Path("output/geometry_compromise_desi")
...
print("GC_DESI_Z",zvals,flush=True)
...
for label,m in MODELS.items():
    ...
    print("GC_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("GC_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        rows.append(row)
        print("GC_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("GC_DESI_TOTAL",label,total,flush=True)
    return rows,total
