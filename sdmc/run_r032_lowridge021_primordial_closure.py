#!/usr/bin/env python3
"""
Primordial closure around the exact-promoted lowridge021 structural solution.
...
OUT=Path("output/lowridge021_primordial_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(tag,ns,tau,Q):
    ...
    print("LOWRIDGE_PRIM_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"lowridge021_primordial_closure.csv",index=False)
...
(OUT/"lowridge021_primordial_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("LOWRIDGE_PRIM_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LOWRIDGE_PRIM_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
