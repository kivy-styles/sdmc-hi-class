#!/usr/bin/env python3
"""
Bayesian structural recovery around the SN-aware sobol072 late background.
...
OUT=Path("output/snneutral021_structural_bo"); OUT.mkdir(parents=True,exist_ok=True)
...
AL=0.022918577727396; BL=0.0129271923275664
...
NEW_BAO=13.22025413059403
SN_PP=-0.0402146861888468
SN_U3=0.0480041236151009
SN_D5=0.0323204100131988
...
sam=qmc.Sobol(d=4,scramble=True,seed=107041)
...
def fit_gp(rows):
    ...
    gp=GaussianProcessRegressor(kernel=ker,normalize_y=True,n_restarts_optimizer=3,random_state=107042)
    gp.fit(Xn,y)
    return gp,float(y.min())

# Sequential expected-improvement Bayesian optimization.
rng=np.random.default_rng(107043)
for it in range(20):
...
df=pd.DataFrame(rows); df.to_csv(OUT/"snneutral021_structural_bo.csv",index=False)
...
summary={"background":{"H0":H0,"A":AL,"B":BL},"Q":Q,"n_ok":int(len(ok)),
         "best":best,"target":"exact promotion needs P+D fair below about -0.03232 to close at least two SN datasets; BO optimizes Planck-lite while raw DESI is checked on promoted points"}
(OUT/"snneutral021_structural_bo_summary.json").write_text(json.dumps(summary,indent=2))
