...
OUT=Path("output/partial_zeq_best_fullplik")
...
CANDIDATES=[
    dict(id="partial_zeq_best",H0=70.66324280546841,ob=0.022039097239759515,
         oc=0.12364444452954193,ns=0.9625227132590487,tau=0.055202901571989066,
         lnAs=3.0598696043919773,AF=0.0590625,
         zc=2.775,width=0.5943750000000001,dfloor=0.04203125,
         lam=18.162499999999998,zt=17.1375),
]
def table(path):
    import re
...
for c in CANDIDATES:
    ...
    if cp.returncode:
        print("PZEQBEST_CLASS_FAIL",c["id"],cp.stdout[-3000:],flush=True)
    ...
    print("PZEQBEST_STABILITY",json.dumps(rec,sort_keys=True),flush=True)
...
def run_one(label,path,seeds):
    ...
    for i,seed in enumerate(seeds):
        ...
        print("PZEQBEST_FULLPLIK_SEED",label,i,row[0],row[2],row[3],flush=True)
    ...
    print("PZEQBEST_FULLPLIK_RESULT",json.dumps(row,sort_keys=True),flush=True)
    return row,x
...
for c in CANDIDATES:
    ...
    rows.append(row)
    print("PZEQBEST_FULLPLIK_DELTA",c["id"],row["delta_chi2_vs_lcdm"],flush=True)

pd.DataFrame(rows).to_csv(OUT/"partial_zeq_best_fullplik.csv",index=False)
...
(OUT/"partial_zeq_best_fullplik_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQBEST_FULLPLIK_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
