...
OUT=Path("output/lowridge021_q_fine_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("LOWRIDGE_QFINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"lowridge021_q_fine_closure.csv",index=False)
...
(OUT/"lowridge021_q_fine_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("LOWRIDGE_QFINE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LOWRIDGE_QFINE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
