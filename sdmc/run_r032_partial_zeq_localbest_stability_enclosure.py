...
OUT=Path("output/partial_zeq_localbest_stability_enclosure"); OUT.mkdir(parents=True,exist_ok=True)
...
LOW=np.array([0.09,0.048,2.70,0.50,0.032,18.00,16.75],float)
HIGH=np.array([0.17,0.058,3.05,0.60,0.045,18.60,17.20],float)
...
print("PZEQBOUND_TARGET",json.dumps({**der0,**stab0,**base},sort_keys=True),flush=True)
...
def candidate(tag,f,af,zc,w,df,lam,zt):
    ...
    if len(vals)<2:
        ...
        print("PZEQBOUND_POINT",json.dumps(rec,sort_keys=True),flush=True)
        return rec
...
df=pd.DataFrame(rows); df.to_csv(OUT/"partial_zeq_localbest_stability_enclosure.csv",index=False)
ok=df[(df.status=="OK") & (df.stable_subluminal==True)].copy()
best=ok.nsmallest(10,"chi2_planck").to_dict("records")
print("PZEQBOUND_BEST_PLANCK",json.dumps(best,sort_keys=True),flush=True)
summary={"target_ellA":TARGET,"center":rows[0],
         "best":best[0] if best else None,
         "best_delta_vs_geom":float(ok.delta_planck_vs_geom.min()) if len(ok) else None}
(OUT/"partial_zeq_localbest_stability_enclosure_summary.json").write_text(json.dumps(summary,indent=2))
print("PZEQBOUND_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
