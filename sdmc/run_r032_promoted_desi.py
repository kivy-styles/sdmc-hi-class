...
MODELS={
    ...
    "ACOUSTIC_ONLY":dict(
        ...
        As=math.exp(3.0773358772153037)/1e10,
        ns=0.9637720508351921,
        tau=0.06456347454525531,
        A_lens=0.0,z_lens=1.5,sigma_lens=0.8,
    ),
    "BRAID_BEST":dict(
        kind="sdmc",
        H0=69.7095092787552,
        ...
        ns=0.9637720508351921,
        tau=0.06456347454525531,
        A_lens=0.024,z_lens=1.5,sigma_lens=0.8,
    ),
}
...
print("PROMOTED_DESI_Z",zvals,flush=True)
...
def ini_sdmc(root,m):
    ...
    parameters_smg = 0.0715,5.0,1.426,0.34231919445927034,1.0,0.045,{m['A_lens']:.17g},{m['z_lens']:.17g},{m['sigma_lens']:.17g}
...
for label,m in MODELS.items():
    ...
    print("PROMOTED_DESI_CLASS",label,cp.returncode,flush=True)
    if cp.returncode!=0:
...
def model_data(label,m):
    ...
    if m["kind"]=="sdmc":
        ...
        print("PROMOTED_DESI_STABILITY",json.dumps({k:v for k,v in rec.items() if k not in ("bg","series")},sort_keys=True),flush=True)
    return rec
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        rows.append(row)
        print("PROMOTED_DESI_BLOCK",json.dumps(row,sort_keys=True),flush=True)
    print("PROMOTED_DESI_TOTAL",label,total,flush=True)
    return rows,total
...
summary={
    "LCDM":totals["LCDM"],
    "ACOUSTIC_ONLY":totals["ACOUSTIC_ONLY"],
    "BRAID_BEST":totals["BRAID_BEST"],
    "delta_acoustic_minus_lcdm":totals["ACOUSTIC_ONLY"]-totals["LCDM"],
    "delta_braid_minus_lcdm":totals["BRAID_BEST"]-totals["LCDM"],
    "delta_braid_minus_acoustic":totals["BRAID_BEST"]-totals["ACOUSTIC_ONLY"],
}
pd.DataFrame(allrows).to_csv(OUT/"promoted_desi_profile.csv",index=False)
(OUT/"promoted_desi_summary.json").write_text(json.dumps(summary,indent=2))
print("PROMOTED_DESI_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
