...
OUT=Path("output/local002_ordinary_edge_hypercube"); OUT.mkdir(parents=True,exist_ok=True)
...
print("LOCAL002_EDGE_LCDM",base,flush=True)

CENTER=np.array([
    70.5653567390982,0.022011983189284802,0.12404331885203719,
    0.9625227132590487,0.055202901571989066,3.0598696043919773,
    0.06017362505197525,2.827464461401105,0.5514493708219379
],float)
...
LOW=np.array([70.42,0.02172,0.12370,0.9615,0.0525,3.058,0.0575,2.68,0.51],float)
HIGH=np.array([70.72,0.02208,0.12435,0.9668,0.0610,3.078,0.0630,2.98,0.60],float)

EDGE1=np.array([70.5653567390982,0.02188,0.12404331885203719,
                0.9635,0.0565,3.068,
                0.06017362505197525,2.827464461401105,0.5514493708219379],float)
EDGE2=np.array([70.54,0.02182,0.12395,
                0.9648,0.0580,3.072,
                0.0605,2.80,0.545],float)

anchors=[("local002_expansion_best",CENTER),("highAs_lowOb",EDGE1),("edge_combo",EDGE2)]
sob=qmc.Sobol(d=9,scramble=True,seed=202609212)
design=anchors+[(f"edge{i+1:03d}",v) for i,v in enumerate(qmc.scale(sob.random_base2(m=5),LOW,HIGH))]
print("LOCAL002_EDGE_DESIGN_SIZE",len(design),flush=True)
print("LOCAL002_EDGE_BOUNDS",json.dumps({"low":LOW.tolist(),"high":HIGH.tolist()}),flush=True)
...
for i,(tag,v) in enumerate(design,1):
    ...
    rows.append(rec); print("LOCAL002_EDGE_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"local002_ordinary_edge_hypercube.csv",index=False)
...
for col,label in [("delta_planck","PLANCK"),("joint_pp","PP"),("joint_union3","UNION3"),("joint_desy5","DESY5")]:
    print("LOCAL002_EDGE_BEST_"+label,ok.nsmallest(12,col).to_dict("records"),flush=True)
