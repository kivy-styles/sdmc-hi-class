#!/usr/bin/env python3
"""
Lower-width extension around the lrlowaf001 structural screen winner.

Motivation:
  The lrref008 lower-A_F extension found lrlowaf001 with a further 0.4569
  chi2 Planck-lite improvement, but its width=0.34276 lies almost on that
  scan's lower width boundary of 0.340. Extend the transition to narrower
  widths while jointly varying A_F, z_c, D_floor and z_t.
...
OUT=Path("output/r032_lrlowaf001_lowwidth_extension"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=.0025; OR=4.17998772e-5
TARGET=301.6798271349195

H00=70.79285859128709
...
AF0=0.03385626635421067
ZC0=3.6682934215292335
W0=0.34275636240839963
D0=0.34231919445927034
DF0=0.04743026036769152
LAM=18.40625
ZT0=16.24762099627405

# A_F, z_c, width, D_floor, z_t
LOW=np.array([0.0275,3.35,0.235,0.0420,15.95],float)
HIGH=np.array([0.0375,4.05,0.360,0.0600,16.45],float)
...
def candidate(tag,af,zc,w,df,zt):
    ...
    if pair is None:
        if not vals:
            ...
            print("LRLOWW_POINT",json.dumps(rec,sort_keys=True),flush=True); return rec
    ...
    print("LRLOWW_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec

rows=[]
base=candidate("lrlowaf001",AF0,ZC0,W0,DF0,ZT0)
if base.get("status")!="OK": raise RuntimeError("lrlowaf001 baseline failed")
BASE=float(base["chi2_planck"]); rows.append(base)

anchors=[
 ("w033",AF0,ZC0,0.330,DF0,ZT0),
 ("w032",0.0335,3.67,0.320,0.0480,16.24),
 ("w030",0.0330,3.70,0.300,0.0490,16.22),
 ("w028",0.0325,3.73,0.280,0.0505,16.20),
 ("w026",0.0320,3.78,0.260,0.0520,16.17),
 ("w024",0.0315,3.82,0.240,0.0540,16.14),
]
for a in anchors:
    rows.append(candidate(*a))

sam=qmc.Sobol(d=5,scramble=True,seed=20260927)
...
for r in rows:
    if r.get("status")=="OK":
        r["delta_planck_vs_lrlowaf001"]=r["chi2_planck"]-BASE
        r["delta_high_vs_lrlowaf001"]=r["chi2_high"]-base["chi2_high"]
        r["delta_lowT_vs_lrlowaf001"]=r["chi2_lowT"]-base["chi2_lowT"]
        r["delta_lowE_vs_lrlowaf001"]=r["chi2_lowE"]-base["chi2_lowE"]
        r["delta_lensing_vs_lrlowaf001"]=r["chi2_lensing"]-base["chi2_lensing"]
        r["delta_cal_vs_lrlowaf001"]=r["chi2_cal"]-base["chi2_cal"]

df=pd.DataFrame(rows)
df.to_csv(OUT/"r032_lrlowaf001_lowwidth_extension.csv",index=False)
...
summary={
 ...
 "best_delta_vs_lrlowaf001":float(ok.chi2_planck.min()-BASE) if len(ok) else None,
}
(OUT/"r032_lrlowaf001_lowwidth_extension_summary.json").write_text(json.dumps(summary,indent=2))
print("LRLOWW_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LRLOWW_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
