...
models={
  'COMBINED':model_data('desi_combined',69.7095092787552,0.02211778594478965,0.1258596659367904,0.9637720508351921),
...
def profile_model(label,m):
    ...
    for b in blocks:
        ...
        for iseed,seed in enumerate(seeds):
            ...
            trials.append(opti)
            print('COMBINED_DESI_TRIAL',label,b['namespace'],iseed,float(opti.fun),
        ...
        rows.append(row)
        print('COMBINED_DESI_BLOCK',json.dumps(row,sort_keys=True),flush=True)
    print('COMBINED_DESI_TOTAL',label,total,flush=True)
    return rows,total
...
summary={
  'combined_chi2':totals['COMBINED'],
  'acoustic_chi2':totals['ACOUSTIC_ONLY'],
  'lcdm_chi2':totals['LCDM'],
  'delta_combined_minus_lcdm':totals['COMBINED']-totals['LCDM'],
  'delta_acoustic_minus_lcdm':totals['ACOUSTIC_ONLY']-totals['LCDM'],
  'delta_combined_minus_acoustic':totals['COMBINED']-totals['ACOUSTIC_ONLY'],
}
print('COMBINED_DESI_SUMMARY',json.dumps(summary,sort_keys=True),flush=True)
pd.DataFrame(allrows).to_csv(OUT/'combined_desi_dr1_fs_profile.csv',index=False)
pd.DataFrame([summary]).to_csv(OUT/'combined_desi_dr1_fs_summary.csv',index=False)
