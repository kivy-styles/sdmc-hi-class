...
OUT=Path("output/retimed_prim"); OUT.mkdir(parents=True,exist_ok=True)
TCMB=2.7255; CAL_SIGMA=0.0025
H0=70.8514; OB=0.02239952; OC=0.12444227328918850; TAU=0.0544
AF=0.060; ZC=3.5; WIDTH=0.8
...
print("RETIMED_PRIM_LCDM",base,flush=True)
...
for ns in NSS:
  for lnAs in LNAS:
    ...
    rows.append(rec); print("RETIMED_PRIM_POINT",json.dumps(rec,sort_keys=True),flush=True)

df=pd.DataFrame(rows); df.to_csv(OUT/"retimed_gravity_primordial_profile.csv",index=False)
ok=df[(df.status=="OK")&(df.stable_subluminal==True)].copy()
print("RETIMED_PRIM_BEST_TOTAL",ok.nsmallest(20,"chi2_total").to_dict("records"),flush=True)
print("RETIMED_PRIM_BEST_HIGH",ok.nsmallest(20,"chi2_high").to_dict("records"),flush=True)
