#!/usr/bin/env python3
"""
Fine Q closure around the exact-promoted lrref008 structural solution.
...
OUT=Path("output/lrref008_q_fine_closure"); OUT.mkdir(parents=True,exist_ok=True)
...
def run(i,dq):
    ...
    print("LRREF008_QFINE_POINT",json.dumps(rec,sort_keys=True),flush=True)
    return rec
...
df.to_csv(OUT/"lrref008_q_fine_closure.csv",index=False)
...
(OUT/"lrref008_q_fine_closure_summary.json").write_text(json.dumps(summary,indent=2))
print("LRREF008_QFINE_BEST",json.dumps(best,sort_keys=True),flush=True)
print("LRREF008_QFINE_SUMMARY",json.dumps(summary,sort_keys=True),flush=True)
